...
- `test-setup.js` (_trem-ui only_) , imports `@testing-library/jest-dom/vitest` matchers and registers `afterEach(cleanup)` to prevent DOM leakage between tests (vitest does **not** auto-cleanup like Jest does).
...
**Affected workspaces:** `@apps/travelstrem`
...
```bash
CI=true pnpm --filter @apps/travelstrem exec -- npx react-scripts test --watchAll=false --env=jsdom
```
...
3. Run: `CI=true pnpm --filter @apps/travelstrem exec -- npx react-scripts test --watchAll=false --env=jsdom`

---

## 6. Known Limitations

- **Root `pnpm test`** (`turbo run test`) fails because apps with no test files (`backend-api`, `admin-shell`, `trevista-remote`, `auth-trem`) report errors. Either add `--passWithNoTests` to their test scripts or run tests per-workspace.
