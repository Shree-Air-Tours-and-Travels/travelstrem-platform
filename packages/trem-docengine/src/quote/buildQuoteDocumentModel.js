...
export function buildQuoteDocumentModel({
  ...
  const items = Array.isArray(source.items)
    ? source.items.filter((item) => item?.label && item.selected !== false)
    : [];
  const fixed = [
    ...
    ["activitiesPrice", "Activities / experiences"],
    ["mealsPrice", "Meals"],
    ["visaFee", "Visa"],
    ["insuranceFee", "Travel insurance"],
    ["platformFee", "TravelsTREM platform fee"],
    ["serviceFee", "Agent/service fee"],
  ]
    .map(([key, label]) => ({
      label,
      amount: number(source[key]),
      category: key.includes("Fee") ? "fee" : "inclusion",
    }))
    .filter((item) => item.amount > 0);
  const additions = [...fixed, ...items].reduce(
    (sum, item) => sum + number(item.amount ?? item.unitAmount),
    0,
  );
  ...
  return {
    ...
    lines: [...fixed, ...items],
    discount,
    amountPayableNow: number(source.amountPayableNow),
    balanceDueDate: source.balanceDueDate || null,
