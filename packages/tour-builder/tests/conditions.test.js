...
describe("evaluateCondition", () => {
  ...
  it("supports single-field rules with every operator", () => {
    ...
    expect(
      evaluateCondition(values, { field: "packageType", operator: "NOT_EQUALS", value: "custom" }),
    ).toBe(true);
    expect(
      evaluateCondition(values, {
        field: "packageType",
        operator: "IN",
        value: ["fixed_departure", "custom"],
      }),
    ).toBe(true);
    expect(
      evaluateCondition(values, { field: "packageType", operator: "NOT_IN", value: ["custom"] }),
    ).toBe(true);
    ...
    expect(evaluateCondition(values, { field: "count", operator: "GREATER_THAN", value: 2 })).toBe(
      true,
    );
    expect(evaluateCondition(values, { field: "count", operator: "LESS_THAN", value: 2 })).toBe(
      false,
    );
  });
  ...
  it("resolves nested field paths", () => {
    ...
    expect(
      evaluateCondition(values, { field: "pricing.currency", operator: "EQUALS", value: "INR" }),
    ).toBe(true);
  });
  ...
  it("defaults unknown operators to EQUALS and never throws", () => {
    ...
    expect(evaluateCondition(null, { field: "a.b.c", operator: "GREATER_THAN", value: 1 })).toBe(
      false,
    );
  });
});
