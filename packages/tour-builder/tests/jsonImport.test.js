            ...
            { key: "meals", type: "TAGS", path: "meals" },
            {
              ...
              itemWidgets: [
                ...
                { key: "d2", type: "NUMBER", path: "pricing.min" },
                {
                  ...
                  type: "OBJECT",
                  path: "nested",
                  widgets: [{ key: "n1", type: "DATE", path: "at" }],
                },
              ],
            },
          ],
...
describe("applyPastedJson", () => {
  it("merges owned fields, normalizing dates inside repeater items", () => {
    ...
    expect(result.appliedKeys).toEqual(
      expect.arrayContaining(["title", "period.days", "startDate", "departures"]),
    );
  ...
  it("reports keys that belong to other steps instead of applying them", () => {
    const result = applyPastedJson(
      definition,
      { title: "ok", commercial: { currency: "INR" } },
      {},
    );
  ...
  it("never applies pasted values for server-managed fields — even empty overrides", () => {
    ...
    const wipe = applyPastedJson(
      managedDef,
      { title: "ok", agentRef: "", providerName: null },
      current,
    );
    ...
    const hostile = applyPastedJson(
      managedDef,
      { agentRef: "agent-hacked", providerName: "Spoofed Ltd" },
      current,
    );
    ...
    const collectionResult = applyPastedJson(
      collectionManaged,
      { agentRef: "nope" },
      { agentRef: "kept" },
    );
