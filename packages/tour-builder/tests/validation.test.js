...
describe("validateValue", () => {
  it("flags empty values only for required widgets", () => {
    ...
    expect(validateValue({ ...widget, validation: [], required: true }, "")).toEqual([
      "Title is required",
    ]);
  });
  ...
  it("ignores null and malformed backend rules without crashing", () => {
    const widget = {
      ...
      label: "Title",
      required: true,
      validation: [null, false, "REQUIRED"],
    };
  ...
  it("enforces length, range, integer and pattern rules", () => {
    ...
    expect(
      validateValue(widget({ type: "MIN_LENGTH", value: 3, message: "too short" }), "ab", {}),
    ).toEqual(["too short"]);
    expect(
      validateValue(widget({ type: "MAX_LENGTH", value: 2, message: "too long" }), "abc", {}),
    ).toEqual(["too long"]);
    expect(validateValue(widget({ type: "MIN", value: 1, message: "too low" }), 0, {})).toEqual([
      "too low",
    ]);
    expect(validateValue(widget({ type: "MAX", value: 10, message: "too high" }), 11, {})).toEqual([
      "too high",
    ]);
    expect(
      validateValue(widget({ type: "INTEGER_MIN", value: 0, message: "bad int" }), -1.5, {}),
    ).toEqual(["bad int"]);
    expect(
      validateValue(
        widget({ type: "PATTERN", value: "^[a-z]+$", message: "lowercase only" }),
        "ABC",
        {},
      ),
    ).toEqual(["lowercase only"]);
  });

  it("validates list sizes and cross-field comparisons", () => {
    const widget = (rule) => ({ key: "w", path: "p", label: "P", validation: [rule] });
    expect(
      validateValue(widget({ type: "MIN_ITEMS", value: 1, message: "need one" }), [], {}),
    ).toEqual(["need one"]);
    expect(
      validateValue(widget({ type: "MAX_ITEMS", value: 1, message: "too many" }), [{}, {}], {}),
    ).toEqual(["too many"]);
    const gte = {
      key: "max",
      path: "max",
      label: "Max",
      validation: [{ type: "GTE_PATH", path: "min", message: "max < min" }],
    };
  ...
  it("checks enabled-package counts and day sequencing", () => {
    ...
    expect(
      validateValue(
        { key: "pk", path: "packages", label: "Packages", validation: [rule] },
        packages,
        {},
      ),
    ).toEqual([]);
    ...
    const days = {
      ...
      path: "itinerary",
      label: "Itinerary",
      validation: [{ type: "DAY_SEQUENCE", message: "days must be sequential" }],
    };
...
describe("validateWidgets", () => {
  it("keys errors by absolute path including nested objects and repeaters", () => {
    const definition = [
      { key: "title", type: "TEXT", path: "title", label: "Title", required: true },
      {
        ...
        path: "pricing",
        widgets: [
          { key: "currency", type: "SELECT", path: "currency", label: "Currency", required: true },
        ],
      },
      {
        key: "items",
      ...
      },
      {
        ...
        path: "departures",
        itemWidgets: [
          { key: "status", type: "SELECT", path: "status", label: "Status", required: true },
        ],
      },
    ];
    const values = {
  ...
  it("validates nested object values and skips backend-hidden fields", () => {
        ...
        widgets: [
          {
            ...
            type: "NUMBER",
            path: "feePercent",
            validation: [{ type: "MAX", value: 100, message: "too high" }],
            visibleWhen: {
    ...
    const values = {
      commercial: { pricingPolicy: { feeType: "PERCENTAGE", feePercent: 101, feeAmountMinor: "" } },
    };
    expect(validateWidgets(definition, values)).toEqual({
      "commercial.pricingPolicy.feePercent": ["too high"],
    });
  });
});
