...
const toTimeInput = (iso) => (iso && String(iso).includes("T") ? String(iso).split("T")[1]?.slice(0, 5) : "");
...
export const TagsWidget = ({ widget, value = [], onChange, error }) => {
    ...
    return (
        <FieldShell widget={widget} error={error}>
            <div className={`tb-tags${listVariant ? ` tb-tags--list tb-tags--${widget.tone || "positive"}` : ""}`}>
                {items.map((item, index) => (
                    <span className="tb-tags__chip" key={`${item}-${index}`}>
                        {listVariant ? <span className="tb-tags__marker" aria-hidden="true">{widget.tone === "negative" ? "−" : "✓"}</span> : null}
                        <span className="tb-tags__text">{String(item)}</span>
                        {editable && (
                            <button type="button" aria-label={`${widget.removeLabel || "Remove"} ${String(item)}`} onClick={() => onChange(widget.path, items.filter((_, idx) => idx !== index))}>×</button>
                        )}
                    </span>
                ))}
                {editable ? <div className="tb-tags__composer">
                    <FormInput
                        type="text"
                        value={draft}
                    ...
                    />
                    {listVariant ? <button className="tb-tags__add" type="button" disabled={!draft.trim()} onClick={addDraft}>{widget.addLabel || "Add"}</button> : null}
                </div> : null}
            </div>
        </FieldShell>
    );
