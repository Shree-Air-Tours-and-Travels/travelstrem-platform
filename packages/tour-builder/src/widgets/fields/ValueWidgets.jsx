...
export const ReadOnlyObjectWidget = ({ widget, value }) => (
    <FieldShell widget={widget} error={null}>
        <dl className="tb-readonly">
            ...
            {!Object.keys(value || {}).length && <dd className="tb-readonly__empty">No values recorded yet.</dd>}
        </dl>
    </FieldShell>
);
    ...
    <FieldShell widget={widget} error={error}>
        <FileUploader
            ...
            transport={async ({ files, onProgress }) => {
                if (typeof uploader?.upload !== "function") throw new Error("Image upload is not available in this portal.");
            ...
            renderFileActions={widget.coverPath !== null && !widget.readOnly
                ? (url) => <button type="button" className="tb-gallery__cover" onClick={() => onChange(widget.coverPath || "photo", url)}>Set cover</button>
                : undefined}
        />
    </FieldShell>
);
