...
export default function RepeaterWidget({ widget, root, basePath, onChange }) {
  ...
  const hasErrorAt = (index) =>
    Object.keys(itemErrors).some((key) => key.startsWith(`${listPath}.${index}.`));
  ...
  return (
    <div className="tb-repeater" id={`tb-widget-${widget.key}`}>
      <header className="tb-repeater__head">
        <span className="tb-repeater__title">
          {widget.label}
      ...
      {(widget.validation || [])
        .filter((rule) => rule && rule.type === "MIN_ITEMS")
        .map((rule) =>
          items.length < Number(rule.value) ? (
            <small key={rule.message} className="tb-field__error tb-repeater__rule">
              {rule.message}
            </small>
          ) : null,
        )}
      ...
      {!items.length && (
        <div className="tb-repeater__empty">{`No ${String(widget.label || "entries").toLowerCase()} yet.`}</div>
      )}
      ...
      <div className="tb-repeater__list">
        {items.map((item, index) => {
          ...
          const labelled =
            typeof widget.itemTitle === "function"
                ...
                {!widget.readOnly && (
                  <div className="tb-card__ops" aria-label={`${title} actions`}>
                    <Button
                      ...
                      size="small"
                      variant="text"
                      primaryClassName="tb-card__op"
                      text="Up"
                      disabled={index === 0}
                      onClick={() => writeList(moveItem(items, index, index - 1))}
                    />
                    <Button
                      type="button"
                      size="small"
                    ...
                    <Button
                      ...
                      text="Remove"
                      iconLeft="x"
                      onClick={() => writeList(items.filter((_, idx) => idx !== index))}
                    />
                  </div>
                )}
              </header>
