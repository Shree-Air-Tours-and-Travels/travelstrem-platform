...
export default function DerivedPricingWidget({ widget, root, basePath, runtime }) {
  ...
  const currency =
    previewState.data?.price?.currency ||
  ...
  const modeLabel =
    derived.displayMode === "FINAL"
      ? copy.finalMode?.label
      : derived.displayMode === "STARTING_FROM"
        ? copy.startingMode?.label
  ...
  return (
    <FieldShell widget={widget} error={null}>
      <div className={`tb-derived${previewState.loading ? " is-loading" : ""}`}>
        <header className="tb-derived__hero">
          ...
          <div className="tb-derived__status" role="status">
            {previewState.loading ? (
              <>
                <Spinner size={18} /> {copy.updating?.label}
              </>
            ) : previewState.data ? (
              copy.ready?.label
            ) : (
        ...
        {previewState.error && (
          <div className="tb-derived__error" role="alert">
            {previewState.error}
          </div>
        )}
            ...
            {packages.map((item) => (
              <article className="tb-derived__package" key={item.packageKey}>
                ...
                <dl className="tb-derived__breakdown">
                  <div>
                    <dt>{copy.supplierComponents?.label}</dt>
                    <dd>{formatMinor(item.costTotalMinor, currency)}</dd>
                  </div>
                  <div>
                    <dt>{copy.agentFee?.label}</dt>
                    <dd>{formatMinor(item.agentFeeMinor, currency)}</dd>
                  </div>
                  <div>
                    <dt>{copy.gst?.label}</dt>
                    <dd>{formatMinor(item.agentGstMinor, currency)}</dd>
                  </div>
                  <div className="tb-derived__equation">
                    <dt>{copy.finalAmount?.label}</dt>
                    <dd>{formatMinor(item.sellingTotalMinor, currency)}</dd>
                  </div>
                </dl>
              </article>
            ))}
