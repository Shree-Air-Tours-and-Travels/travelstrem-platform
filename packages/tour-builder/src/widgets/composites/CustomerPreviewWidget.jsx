...
const formatTourPrice = (amount, currency = "INR") =>
...
export default function CustomerPreviewWidget({ widget, root }) {
  ...
  const pricePrefix =
    commercial.displayMode === "STARTING_FROM"
      ? `${copy.from?.label || ""} `
      : commercial.displayMode === "ESTIMATED"
        ? `${copy.approximate?.label || ""} `
        : "";
        ...
        <div className="tb-preview__identity">
          ...
          <p>
            <span>
              ...
              {[preview.city?.from, preview.city?.to].filter(Boolean).join(" → ") ||
                copy.destinationPending?.label}
            </span>
            <span>
              <Icon name="calendarDays" size={16} /> {duration}
            </span>
          </p>
        </div>
      ...
      {packages.length > 0 && (
        <div className="tb-preview__grid">
          {packages.map((pkg) => (
            <article
              key={pkg.packageKey}
              className={`tb-card tb-preview__card${pkg.recommended ? " is-recommended" : ""}`}
            >
              <header className="tb-preview__card-head">
                ...
                {pkg.recommended && (
                  <span className="tb-preview__badge">
                    <Icon name="star" size={12} /> {copy.recommended?.label}
                  </span>
                )}
              </header>
              <strong className="tb-preview__package-price">
                {pricePrefix}
                {formatPackagePrice(
                  pkg.pricing?.sellingTotalMinor,
                  commercial.currency,
                  copy.pricePending?.label,
                )}
              </strong>
              {pkg.description && <p className="tb-preview__desc">{pkg.description}</p>}
              {!!pkg.includedComponentKeys?.length && (
                <section className="tb-preview__items">
                  <h5>{copy.included?.label}</h5>
                  <ul>
                    {pkg.includedComponentKeys.map((key) => (
                      <li key={key}>
                        <span className="tb-preview__item-icon">
                          <Icon name="check" size={13} />
                        </span>
                        {componentsById[key] || key}
                      </li>
                    ))}
                  </ul>
                </section>
              )}
              {!!pkg.optionalComponentKeys?.length && (
                <section className="tb-preview__items is-optional">
                  <h5>{copy.optional?.label}</h5>
                  <ul>
                    {pkg.optionalComponentKeys.map((key) => (
                      <li key={key}>
                        <span className="tb-preview__item-icon">
