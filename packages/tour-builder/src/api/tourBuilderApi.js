...
const unwrapEnvelope = (response) => {
    const payload = response?.builder ? response : (response?.component?.data || response?.data || response);
    return payload;
};
...
const hydrateLabelRefs = (value, labels = {}) => {
    ...
    Object.entries(value).forEach(([key, child]) => {
        ...
        if (key.endsWith("Ref") && typeof child === "string" && Object.prototype.hasOwnProperty.call(labels, child)) {
