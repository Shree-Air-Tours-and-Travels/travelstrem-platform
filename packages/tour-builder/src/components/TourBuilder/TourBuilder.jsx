...
export default function TourBuilder({
    ...
    const builder = useTourBuilder({ tourId, startStepKey, onExit, onComplete, onLocationChange, trackPosition: !viewOnly });
    ...
    useEffect(() => {
        let active = true;
        tourBuilderApi.fetchDefinition()
        ...
        return (
            <div className="tb-shell">
                <EmptyState
                    ...
                    action={<Button text="Retry" variant="solid" color="primary" onClick={() => builder.reload()} />}
                />
            </div>
        );
    }
    ...
    const progress = Math.max(0, Math.min(100, Number(builder.meta?.process?.progress?.percentage || 0)));
    ...
    return (
        <>
            <WizardFormShell
                ...
                steps={(overview.steps || []).map((step) => ({ id: step.stepKey, title: step.title, description: step.description, disabled: step.readOnlyStep }))}
                ...
                className="tb-shell"
            >
                {builder.error && <div className="tb-banner tb-banner--error" role="alert">{builder.error}</div>}
                <StepWorkspace
            ...
            <ConfirmOverlay
                ...
                onConfirm={() => { setConfirmExit(false); builder.exit(); }}
            ...
            <ConfirmOverlay
                ...
                onConfirm={() => { const target = pendingStepKey; setPendingStepKey(null); builder.goTo(target); }}
...
function StepWorkspace({ builder, uploader, onRequestExit, onDirtyChange, forceReadOnly = false, mode = "create", modeCopy = {} }) {
    ...
    useEffect(() => {
        ...
        form.setErrors(Object.fromEntries(Object.entries(serverErrors).map(([path, message]) => [path, Array.isArray(message) ? message : [message]])));
    }, [builder.fieldErrors]);
        ...
        const timer = window.setTimeout(async () => {
            ...
            try {
                ...
                if (!previewError.cancelled) {
                    setPricingPreview({ loading: false, data: null, error: previewError.message || "Pricing could not be calculated" });
                }
            }
        }, 450);
    ...
    const readOnly = forceReadOnly || !!builder.definition.readOnlyStep || builder.permissions?.canEdit === false;
    ...
    const finishLabel = stayOnStep ? (modeCopy.primaryAction || "Save changes") : (builder.definition.nextActionLabel || (showNext ? "Save & continue" : "Finish"));
    ...
    return (
        <>
            ...
            {(localError || builder.error) && (
                <div className="tb-banner tb-banner--error" role="alert">{localError || builder.error}</div>
            )}
            ...
            <FloatingActionBar
                ...
                actions={forceReadOnly ? [
                    { label: "Exit review", variant: "ghost", iconLeft: "logout", align: "left", onClick: onRequestExit },
                ] : [
                    ...(showBack
                        ? [{ label: "Back", variant: "outline", iconLeft: "chevronLeft", align: "left", disabled: saving, onClick: () => persist("back") }]
                        : (actions.exit !== false ? [{ label: "Exit", variant: "ghost", iconLeft: "logout", align: "left", disabled: saving, onClick: onRequestExit }] : [])),
                    ...(showBack && actions.exit !== false ? [{ label: "Exit", variant: "ghost", iconLeft: "logout", align: "left", overflowMobile: true, disabled: saving, onClick: onRequestExit }] : []),
                    ...(!readOnly ? [{ label: "Paste JSON", variant: "ghost", iconLeft: "sparkles", align: "right", overflowMobile: true, disabled: saving, onClick: () => setShowJsonImport(true) }] : []),
                    ...(actions.cancel ? [{ label: "Cancel", variant: "ghost", iconLeft: "x", align: "right", overflowMobile: true, disabled: saving, onClick: onRequestExit }] : []),
                    [{ label: saving ? "Saving…" : finishLabel, variant: "primary", iconRight: stayOnStep ? "check" : (showNext ? "chevronRight" : "check"), align: "right", primary: true, disabled: saving, onClick: () => persist(stayOnStep ? "stay" : (showNext ? "next" : "done")) }],
                ].flat()}
            />
