...
export default function PasteJsonDialog({ open, definition, currentValues, onClose, onApply }) {
    ...
    const loadTemplate = async (scope) => {
        ...
        try {
            const payload = await tourBuilderApi.fetchTemplate(scope === "full" ? null : definition?.stepKey);
            const body = payload?.tour ? payload : (payload?.data || payload);
            const template = scope === "full"
                ? (body?.tour ?? {})
                : (body?.records ? { [body.recordKey || "records"]: body.records } : (body?.tour ?? {}));
            ...
            setHintsError("Backend template unavailable — showing the field skeleton from this step's form definition instead.");
    ...
    const handleApply = () => {
        setError(null);
        if (!text.trim()) { setError("Paste a JSON object first."); return; }
        let parsed;
        try {
            ...
            <div className="tb-json-dialog__body" role="dialog" aria-modal="true">
                <header className="tb-json-dialog__head">
                    <div>
                        ...
                        <p>
                            ...
                            {" "}<strong>{definition?.title || "this step"}</strong> are applied; everything else is listed as skipped.
                            Amounts use integer paise.
                        </p>
                    </div>
                    <Button type="button" variant="text" primaryClassName="btn" iconLeft="x" text="" onClick={onClose} aria-label="Close" />
                </header>
                ...
                <footer className="tb-json-dialog__foot">
                    ...
                    <div className="tb-json-dialog__actions">
                        <Button type="button" variant="text" color="primary" primaryClassName="btn" text="Clear" disabled={!text} onClick={() => { setText(""); setError(null); setHints(null); setRules([]); }} />
                        <Button
                            type="button"
                            variant="solid"
                            color="primary"
