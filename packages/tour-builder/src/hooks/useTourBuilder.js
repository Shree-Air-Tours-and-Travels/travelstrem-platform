 ...
 */
export default function useTourBuilder({ tourId: initialTourId = null, startStepKey = null, onExit, onComplete, onLocationChange, trackPosition = true }) {
    ...
    const loadStep = useCallback(async (stepKey, targetTourId) => {
        setLoading(true);
        setError(null);
        setFieldErrors({});
        ...
        try {
            const envelope = await tourBuilderApi.loadStep({ tourId: targetTourId ?? undefined, stepKey, signal: controller.signal });
    ...
    const saveStep = useCallback(async ({ stepKey, data, direction }) => {
        setSaving(true);
        setError(null);
        setFieldErrors({});
        try {
            ...
            const nextKey = direction === "back"
                ? result.previousStepKey
                : result.nextStepKey;
            if (nextKey) {
    ...
    const goTo = useCallback(async (stepKey) => {
        if (!stepKey) return;
