...
export default function useStepForm({ definition, initialValues }) {
    ...
    const arrayOps = useCallback((path) => ({
        append: (item) => setValues((current) => {
            const list = [...(getPath(current, path) || []), item];
            setDirty(true);
            return updatePath(current, path, list);
        }),
        replace: (index, item) => setValues((current) => {
            const list = (getPath(current, path) || []).map((existing, idx) => (idx === index ? item : existing));
            setDirty(true);
            return updatePath(current, path, list);
        }),
        remove: (index) => setValues((current) => {
            const list = (getPath(current, path) || []).filter((_, idx) => idx !== index);
            setDirty(true);
            return updatePath(current, path, list);
        }),
        duplicate: (index) => setValues((current) => {
        ...
        reorder: (from, to) => setValues((current) => {
    ...
    const visibleWidgetCount = useMemo(() => widgetsByStep.filter((widget) => evaluateCondition(values, widget.visibleWhen)).length, [widgetsByStep, values]);

    const resetBaseline = useCallback((nextValues) => {
