...
export const unwrapTourJson = (value, depth = 0) => {
    ...
    for (const key of ENVELOPE_KEYS) {
        ...
        if (Array.isArray(candidate) && candidate.length === 1) return unwrapTourJson(candidate[0], depth + 1);
        if (candidate && !Array.isArray(candidate) && typeof candidate === "object") return unwrapTourJson(candidate, depth + 1);
    }
    return value;
};
...
const SKIP_TYPES = [WIDGET_TYPES.PACKAGE_COMPOSER, WIDGET_TYPES.DERIVED_PRICING, WIDGET_TYPES.CUSTOMER_PREVIEW];
...
const indexWidgets = (definition) => {
    ...
    const visit = (widgets, prefix) => (widgets || []).forEach((widget) => {
            ...
            (widget.itemWidgets || []).forEach((child) => {
                ...
                if (child.type === WIDGET_TYPES.OBJECT) {
                    (child.widgets || []).forEach((grandchild) => {
                        if (grandchild?.path) leaves.set(`${abs}.${child.path}.${grandchild.path}`, grandchild);
                    });
                    return;
                }
...
export const buildStepTemplate = (definition) => {
    ...
    (definition?.substeps || []).forEach((substep) => substep.children.forEach((child) => (
        buildObjectSkeleton(child.widgets, template)
    )));
    return template;
};
...
export const serverManagedPaths = (definition) => {
    ...
    const walk = (widgets = []) => widgets.forEach((widget) => {
        if (!widget?.path) return;
        if (widget.serverManaged) managed.add(widget.path);
        if (widget.type === WIDGET_TYPES.OBJECT) walk(widget.widgets);
        if (widget.type === WIDGET_TYPES.REPEATER || widget.type === WIDGET_TYPES.COLLECTION_REPEATER) walk(widget.itemWidgets);
    });
    (definition?.substeps || []).forEach((substep) => substep.children.forEach((child) => walk(child.widgets)));
    return managed;
};
...
export const applyPastedJson = (definition, pasted, currentValues = {}) => {
    ...
    const owns = (path) => (
        ...
        || leaves.has(path)
        || lists.has(path)
        || ownedPaths.some((owned) => owned === path || path.startsWith(`${owned}.`) || owned.startsWith(`${path}.`))
    );
        ...
        (itemWidgets || []).forEach((widget) => {
            ...
            if (widget.type === WIDGET_TYPES.OBJECT) {
                (widget.widgets || []).forEach((child) => {
                    if (child?.path) next = coerceItemField(next, { ...child, path: `${widget.path}.${child.path}` });
                });
                return;
            }
    ...
    const mergeObject = (source, basePath) => {
        ...
        if (repeater && Array.isArray(incoming)) {
            if (managedPaths.has(abs) || !owns(abs)) { ignoredKeys.push(abs); return; }
            values = setPath(values, abs, incoming.map((item) => sanitizeListItem(item, repeater.itemWidgets)));
        ...
        if (leaf && !Array.isArray(incoming) && (incoming == null || typeof incoming !== "object")) {
            if (managedPaths.has(abs)) { ignoredKeys.push(abs); return; }
            if (!owns(abs)) { ignoredKeys.push(abs); return; }
            const coerced = coerceWidgetValue(leaf, incoming);
            if (coerced === undefined) { ignoredKeys.push(abs); return; }
