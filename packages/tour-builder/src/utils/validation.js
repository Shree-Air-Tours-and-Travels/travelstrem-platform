...
const evaluate = (rule) => {
    ...
    switch (rule.type) {
        ...
        case "INTEGER_MIN": {
            ...
            return rule.actual != null && rule.actual !== "" && (!Number.isSafeInteger(parsed) || parsed < Number(rule.value));
        }
        case "PATTERN":
            try {
                return rule.actual != null && rule.actual !== "" && !(new RegExp(rule.value)).test(String(rule.actual));
            } catch {
                return true;
            }
        case "MIN_ITEMS":
        ...
        case "MAX_ITEMS":
            return Array.isArray(rule.actual) && rule.actual.length > Number(rule.value);
        case "GTE_PATH":
            return rule.actual != null && rule.compared != null && Number(rule.actual) < Number(rule.compared);
        case "ENABLED_COUNT": {
            const enabled = (Array.isArray(rule.actual) ? rule.actual : []).filter((item) => item?.enabled !== false);
        ...
        case "DAY_SEQUENCE": {
            ...
            return days.some((day, index) => Number(day?.day ?? day?.dayNumber ?? index + 1) !== index + 1);
        }
        default:
            return false;
    }
...
export const validateValue = (widget = {}, value, values) => {
    const errors = [];
    const rules = (Array.isArray(widget.validation) ? widget.validation : [])
        .filter((rule) => rule && typeof rule === "object");
    rules.forEach((rule) => {
        if (isEmptyValue(value)) {
            // Empty lists still violate minimum-size rules.
            if (rule.type === "MIN_ITEMS" && Number(rule.value) > 0) {
                errors.push(rule.message || `${widget.label || widget.key} needs at least ${rule.value} entries`);
...
export const validateWidgets = (widgets = [], values = {}, prefix = "", rootValues = values) => {
    ...
    (Array.isArray(widgets) ? widgets : []).forEach((widget) => {
        ...
        if (widget.type === "OBJECT") {
            ...
            Object.assign(errors, validateWidgets(widget.widgets || [], objectValue, childPrefix, rootValues));
        }
        if (widget.type === "REPEATER" || widget.type === "COLLECTION_REPEATER") {
            const items = getPath(values, widget.path) || [];
            items.forEach((item, index) => {
                Object.assign(errors, validateWidgets(widget.itemWidgets || [], item, `${childPrefix}.${index}`, rootValues));
            });
        }
    });
    return errors;
