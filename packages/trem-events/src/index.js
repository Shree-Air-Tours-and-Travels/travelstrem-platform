...
const buildEventConfig = (sessionConfig = {}, headerConfig = {}) => {
    ...
    return {
        ...
        urls: {
            ...
            ...(headerConfig.authActions?.login?.path ? { proceedToLogin: headerConfig.authActions.login.path } : {}),
            ...(headerConfig.authActions?.logout?.redirectTo ? { navigateToLogout: headerConfig.authActions.logout.redirectTo } : {}),
        },
        authorization,
...
export const createPortalEventController = ({
    ...
    const runRedirect = async (eventName, payload = {}, translatedEvent = null) => {
        ...
        const navigationState = buildNavigationState(page || translatedEvent, payload, translatedEvent?.eventData);
    ...
    const dispatch = async (eventName, payload = {}, meta = {}) => {
        ...
        if (type === "translation") {
            ...
            for (const childEvent of childEvents) {
                ...
                const childPayload = mergeEventData(payload, resolveTemplate(childEvent.eventData || {}, payload));
                handled = (await dispatch(childEvent.name, childPayload, { ...meta, type: childEvent.type })) || handled;
            }
