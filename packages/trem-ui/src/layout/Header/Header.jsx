...
const canShowItem = (item, session) => {
  ...
  if (item?.access === "roles")
    return authenticated && Array.isArray(item.roles) && item.roles.includes(role);
  return true;
};
...
export default function Header({
  ...
  const navItems = useMemo(() => {
    const configuredMenu =
      Array.isArray(config.menu) && config.menu.length ? config.menu : config.navigation || [];
    return configuredMenu
      .filter((item) => canShowItem(item, session))
      .map((item) => ({
        ...item,
        items: Array.isArray(item.items)
          ? item.items.filter((child) => canShowItem(child, session))
          : item.items,
      }))
  ...
  const runAction = useCallback((handler, fallbackEventName) => {
    ...
    if (fallbackEventName && typeof window !== "undefined") {
      window.dispatchEvent(
        new CustomEvent(fallbackEventName, { detail: { source: "header-drawer" } }),
      );
    }
  }, []);
  ...
  const NavItem = ({ item, isFirst, drawer, activePath, firstLinkRef, onNavClick, onClose }) => {
    ...
    if (item.type === "dropdown") {
      const isDropdownActive = (item.items || []).some((child) =>
        isPathActive(getNavPath(child), activePath),
      );
        ...
        return (
          <li className="trem-header__drawer-dropdown">
            <Button
              variant="text"
              text={item.label}
              iconLeft={getNavIcon(item)}
              iconRight="chevronDown"
              onClick={() => setExpanded((s) => !s)}
              primaryClassName={`trem-header__drawer-dropdown-trigger${isDropdownActive ? " is-active" : ""}`}
            />
            {isExpanded && (
              <ul className="trem-header__drawer-sublist">
                {item.items.map((child) => {
                  ...
                  return (
                    <li key={child.id}>
                      {child.disabled ? (
                        <span className="trem-header__drawer-link is-disabled">
                        ...
                        <NavLink
                          to={childPath}
                          className={() => (isPathActive(childPath, activePath) ? "active" : "")}
                          onClick={(event) => {
          ...
          <Dropdown
            ...
            trigger={
              <Button
                ...
                primaryClassName={`trem-header__dropdown-trigger${isDropdownActive ? " is-active" : ""}`}
              />
            }
    ...
    return (
      <li>
        {item.disabled ? (
          <span
            className={
              drawer ? "trem-header__drawer-link is-disabled" : "trem-header__link is-disabled"
            }
          >
      ...
      return (
        <li>
          <NavLink
            ...
            onClick={(event) => {
              event.preventDefault();
              onPathClick(loginAction.path || "/login", loginAction.label || "Login");
            }}
      ...
      <>
        {leftSection.welcome && (
          <li className={mobile ? "trem-header__drawer-user" : "trem-header__user"}>
            <span className="trem-header__user-welcome">
  ...
  const renderActions = (wrapItems = true) => {
    const favorites =
      favoritesEnabled && user ? (
        <Button
          variant="text"
          iconLeft="heart"
          primaryClassName="trem-header__action-btn"
    ...
    const profile = (
      <ProfileActionMenu
        ...
        onLogout={onLogout}
        logoutLabel={logoutAction.label || "Logout"}
      />
    );
    ...
    <>
      <header className={`trem-header ${open ? "is-open" : ""} ${className}`.trim()} role="banner">
        <div className="trem-header__container">
          <Button
            variant="text"
            iconLeft={open ? "menuClose" : "menuOpen"}
            onClick={() => setOpen((s) => !s)}
            aria-label={open ? "Close menu" : "Open menu"}
            aria-expanded={open}
            primaryClassName="trem-header__toggle"
          />
          <Button
            variant="text"
            onClick={() => onPathClick(brand.homePath || "/", brand.label || "TravelsTrem")}
            primaryClassName="trem-header__logo"
          >
            <BrandLogo
          ...
          <nav className="trem-header__nav" role="navigation" aria-label="Main navigation">
            <ul className="trem-header__menu trem-header__menu--start">
              {navItems.map((item, i) => (
                <NavItem
                  item={item}
                  key={item.id}
            ...
            <ul className="trem-header__menu trem-header__menu--end">
        ...
        <div
          className={`trem-header__overlay ${open ? "is-visible" : ""}`}
        ...
        />
        <aside
          className={`trem-header__drawer ${open ? "is-open" : ""}`}
          aria-hidden={!open}
          ...
          <div className="trem-header__drawer-inner">
            <div className="trem-header__drawer-top">
              {user ? (
                ...
                <NavLink
                  to={loginAction.path || "/login"}
                  className="trem-header__drawer-login"
                  onClick={(event) => {
                    event.preventDefault();
                    onPathClick(loginAction.path || "/login", loginAction.label || "Login");
                  }}
              ...
              <Button
                ...
                primaryClassName="trem-header__drawer-close"
              />
            </div>
            ...
            <div className="trem-header__drawer-bottom">
              <Button
                ...
                primaryClassName="trem-header__drawer-action"
              />
              <Button
                variant="text"
                iconLeft="settings"
              ...
              {session?.isAuthenticated && (
                <Button
                  variant="text"
                  iconLeft="logout"
                  text={logoutAction.label || "Logout"}
                  onClick={() => runAction(onLogout)}
                  primaryClassName="trem-header__drawer-action trem-header__drawer-action--danger"
                />
              )}
            </div>
          </div>
