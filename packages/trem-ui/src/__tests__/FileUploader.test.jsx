...
describe("FileUploader", () => {
  it("validates type, size, and file-count constraints before transport", () => {
    ...
    const result = validateUploadFiles([valid, invalid], { accept: "image/*", maxFileSize: 20, maxFiles: 2 });
  ...
  it("creates multipart payloads that retain binary File objects and metadata", async () => {
    ...
    const payload = await createFileUploadPayload([file], { fieldName: "documents", metadata: { category: "identity" } });
  ...
  it("supports disabled keyboard state and delegates uploads to the host transport", async () => {
    ...
    const { container } = render(<FileUploader label="Tour photos" accept="image/*" maxFiles={4} multiple transport={transport} onChange={onChange} />);
