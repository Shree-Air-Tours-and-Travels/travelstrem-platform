...
describe('ConfigurableForm', () => {
  ...
  it('collapses a section when toggled and expands when defaultExpanded is false', () => {
    ...
    expect(screen.getByRole('button', { name: /contact details/i })).toHaveAttribute('aria-expanded', 'true');
  ...
  it('collapses an open section on toggle', () => {
    ...
    expect(screen.getByRole('button', { name: /personal information/i })).toHaveAttribute('aria-expanded', 'false');
    expect(screen.queryByText('First name')).not.toBeInTheDocument();
  });

  it('shows field errors', () => {
    render(<ConfigurableForm config={baseConfig} values={{}} errors={{ firstName: 'First name is required' }} onChange={() => {}} />);
      ...
      sections: [
        {
          ...
          title: 'Details',
          fields: [
            { name: 'title', type: 'select', label: 'Title', width: 'auto', options: [{ value: 'Mr', label: 'Mr' }] },
      ...
      sections: [
        {
          ...
          title: 'Details',
          fields: [
            { name: 'title', type: 'select', label: 'Title', options: [{ value: 'mr', label: 'Mr' }, { value: 'ms', label: 'Ms' }] },
            { name: 'gender', type: 'select', label: 'Gender', options: [{ value: 'male', label: 'Male' }, { value: 'female', label: 'Female' }], width: 140 },
            { name: 'nationality', type: 'select', label: 'Nationality', options: [{ value: 'in', label: 'India' }, { value: 'gb', label: 'United States of America' }] },
          ],
        },
      ],
    };
