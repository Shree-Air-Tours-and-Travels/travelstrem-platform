...
describe("DOM-compatible trem form controls", () => {
  it("keeps event.target.value semantics for inputs and text areas", () => {
    const onInput = vi.fn();
    const onText = vi.fn();
    render(<><FormInput ariaLabel="Name" value="" onChange={onInput} /><FormTextArea aria-label="Notes" value="" onChange={onText} /></>);
  ...
  it("converts option children into the standard single select", () => {
    render(<FormSelect label="Tier" value="standard" onChange={() => {}}><option value="standard">Standard</option></FormSelect>);
