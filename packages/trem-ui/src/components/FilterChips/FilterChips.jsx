...
export default function FilterChips({
  ...
  return (
    <div className={`trem-filter-chips ${className}`.trim()} role="region" aria-label={ariaLabel}>
      <div className="trem-filter-chips__items">
        {items.map((item) => (
          <Button
            ...
            variant="outline"
            size="small"
            primaryClassName="trem-filter-chips__chip"
            onClick={() => onRemove?.(item.id)}
            aria-label={`Remove ${displayText(item.label)} filter`}
          >
      ...
      {onClearAll ? (
        <Button type="button" variant="text" size="small" onClick={onClearAll} text={clearLabel} />
      ) : null}
    </div>
  );
}
