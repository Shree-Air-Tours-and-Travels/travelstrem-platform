...
const ContactForm = ({ fieldsMeta = [], formValues = {}, onChange, onSubmit, onCancel, submitting = false, submitText = "Send Request", errors = {}, Button: CustomButton }) => {
  ...
  const renderField = (field) => {
    ...
    if (type === "select") {
      const items = (field.options || []).map((option) => typeof option === "string"
        ? { id: option, value: option, label: option }
        : { id: option.value, ...option });
      return <Dropdown variant="select" items={items} label={field.label} placeholder={field.placeholder || "Select an option"} value={value} onChange={(item) => onChange(field.name, item?.value ?? item?.id ?? "")} error={errors[field.name]} portalClassName="trem-contact-form__dropdown-layer" portalZIndex={2100} />;
    }
    if (type === "textarea") {
  ...
  return (
    <form className="trem-contact-form" noValidate onSubmit={onSubmit}>
      ...
      <div className="trem-contact-form__actions">
        ...
        <Btn type="button" text="Cancel" size="medium" variant="outline" color="primary" onClick={onCancel} disabled={submitting} />
      </div>
    </form>
  );
};

ContactForm.propTypes = { fieldsMeta: PropTypes.array, formValues: PropTypes.object, onChange: PropTypes.func.isRequired, onSubmit: PropTypes.func.isRequired, onCancel: PropTypes.func.isRequired, submitting: PropTypes.bool, submitText: PropTypes.string, errors: PropTypes.object, Button: PropTypes.oneOfType([PropTypes.func, PropTypes.object]) };
