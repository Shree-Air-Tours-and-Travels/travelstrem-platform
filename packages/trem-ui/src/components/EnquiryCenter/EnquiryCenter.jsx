...
const Detail = ({ label, value, wide = false }) => value ? (
...
export default function EnquiryCenter({
  ...
  const records = [...bookings, ...enquiries].map((item) => {
    const recordType = String(item.recordType || (item.bookingRef ? "booking" : "enquiry")).toLowerCase();
    ...
    return {
      ...
      service: {
        ...
        type: item.product ? String(item.product).replace(/^./, (letter) => letter.toUpperCase()) : (item.service?.type || "Tour"),
  ...
  const selected = records.find((item) => item.id === selectedId || item.reference === selectedId) || null;

  if (error && !records.length) return <ErrorState title="Bookings and enquiries could not be loaded" description={error} retry={onRetry} />;

  if (selected) {
    return (
      <section className="trem-enquiries trem-enquiries--detail" aria-labelledby="enquiry-detail-title">
        <div className="trem-enquiries__toolbar">
          <Button text="Back to bookings & enquiries" variant="outline" iconLeft="arrowLeft" onClick={onBack} />
          <StatusBadge value={selected.statusLabel || selected.status} />
        </div>
        <header className="trem-enquiries__hero">
          <div>
            <span className="trem-enquiries__eyebrow">{selected.directionLabel || selected.recordTypeLabel}</span>
            <h1 id="enquiry-detail-title">{selected.service.name}</h1>
            <p>{selected.reference} · {selected.createdDisplay}</p>
          </div>
        </header>

        <div className="trem-enquiries__detail-grid">
          <article className="trem-enquiries__panel">
            <h2>{selected.counterpart?.label || (selected.recordType === "booking" ? "Traveller" : "Contact")}</h2>
            <dl className="trem-enquiries__details">
              ...
              <Detail label="Booking amount" value={selected.amountDisplay || selected.priceDisplay} />
            </dl>
          </article>
          <article className="trem-enquiries__panel">
            ...
            <dl className="trem-enquiries__details">
              ...
              {(selected.request?.hotelRequests || []).map((request) => {
                const budget = request.budgetPerNightMinor == null ? "" : new Intl.NumberFormat("en-IN", { style: "currency", currency: request.currency || "INR", maximumFractionDigits: 0 }).format(Number(request.budgetPerNightMinor) / 100);
                const preference = [request.propertyClass, request.roomType, budget ? `${budget} per room / night` : "", request.requirements].filter(Boolean).join(" · ");
                return <Detail key={request.stayKey} label={`Hotel request · ${request.location || request.stayKey}`} value={preference} wide />;
              })}
              {Object.entries(selected.request?.customizationAnswers || {}).map(([question, answer]) => <Detail key={question} label={question} value={answer} wide />)}
  ...
  return (
    <section className="trem-enquiries" aria-labelledby="enquiries-title">
      ...
      <BookingTable
        ...
        columns={[
          { id: "reference", label: "Reference", minWidth: 145, sortable: true, emphasis: "danger" },
          { id: "service", label: "Tour / service", type: "mediaText", titleAccessor: "service.name", subtitleAccessor: "service.type", mediaAccessor: "service.image", sortAccessor: "service.name", minWidth: 260, sortable: true },
          ...
          { id: "statusDisplay", label: "Status", type: "status", toneAccessor: "statusTone", minWidth: 145 },
          { id: "createdAt", label: "Created", accessor: "createdDisplay", sortAccessor: "createdAt", minWidth: 135, sortable: true },
          { id: "actions", label: "", type: "actions", minWidth: 64, align: "right", actions: [{ id: "view", label: "View details", icon: "eye", onClick: onSelect }] },
        ]}
        rows={records}
        actions={{
          search: { placeholder: "Search reference, tour or person", keys: ["reference", "service.name", "party", "statusDisplay"] },
          filters: [{
            id: "recordType",
            label: "Record type",
            accessor: "recordType",
        ...
        sortingHeader={{
          ...
          defaultValue: "newest",
          options: [
            { label: "Newest", value: "newest", sort: { columnId: "createdAt", direction: "desc" } },
            { label: "Reference", value: "reference", sort: { columnId: "reference", direction: "asc" } },
          ],
        }}
