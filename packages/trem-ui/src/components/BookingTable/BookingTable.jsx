...
function HeroBanner({ config = {} }) {
  ...
  return (
    <header
      className={`booking-table__hero booking-table__hero--${config.variant || "default"}`}
    ...
    return wrapClickableCell(
      <div className="booking-table__media-cell">
        {media ? (
          ...
          <span className="booking-table__media-fallback">{String(title || "").charAt(0)}</span>
        )}
        <span>
          <strong>{title}</strong>
...
export default function BookingTable({
  ...
  const [filters, setFilters] = useState(() =>
    (actions.filters || []).reduce(
      (acc, filter) => ({ ...acc, [filter.id]: filter.value || filter.defaultValue || "all" }),
      {},
    ),
  );
  ...
  const searchableKeys = (
    actions.search?.keys || columns.map((column) => column.accessor || column.id)
  ).filter(Boolean);
  const sortOption = (sortingHeader.options || [])
    .map(normalizeOption)
    .find((option) => option.value === sortValue);
  ...
  const sortedRows = useMemo(
    ...
    [filteredRows, effectiveSort, columns, isServerSide],
  );
    ...
    >
      ...
      <div className="booking-table__panel">
        ...
        <div className="booking-table__scroll" tabIndex={0}>
          <table
            style={{ minWidth: table.contentMinWidth || table.tableMinWidth || table.minWidth }}
          >
            <thead>
              <tr>
                {columns.map((column) => (
                  ...
                  >
                    ...
                    >
                      ...
                      {column.sortable ? (
                        <Icon
                          name={
                            columnSort?.columnId === column.id && columnSort.direction === "desc"
                              ? "chevronDown"
                              : "chevronRight"
                          }
                          size={14}
        ...
        {table.mobileCard ? (
          <div className="booking-table__mobile">
            ...
            {!table.loading && table.error ? (
              <p className="booking-table__mobile-error">{table.error}</p>
            ) : null}
            {!table.loading &&
              !table.error &&
              visibleRows.map((row, rowIndex) => (
