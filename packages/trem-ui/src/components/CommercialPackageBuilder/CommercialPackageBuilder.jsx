...
const money = (minor, currency) => currency
...
export default function CommercialPackageBuilder({
  ...
  const commercial =
    value?.version === "COMPONENTS_V1"
      ? value
      : createCommercialDefaults(period);
  ...
  useEffect(() => {
    const patch = {};
    if (!commercial.currency && currencyOptions[0]?.value) patch.currency = currencyOptions[0].value;
  ...
  const panels = useMemo(() => [
    ...
    ...commercial.components.map((component, index) => ({ id: `component:${component.componentKey}`, label: component.name || `Component ${index + 1}` })),
    ...commercial.packages.map((pkg, index) => ({ id: `package:${pkg.packageKey}`, label: pkg.name || `Package ${index + 1}` })),
  ], [commercial.components, commercial.packages]);
  useEffect(() => {
    if (!panels.some((panel) => panel.id === activePanel)) setActivePanel("basis");
  }, [activePanel, panels]);
  useEffect(() => { setActiveChild(activePanel === "basis" ? "basis" : "details"); }, [activePanel]);
  useEffect(() => { setActiveAssignment((index) => Math.max(0, Math.min(index, commercial.components.length - 1))); }, [commercial.components.length]);
  const activePanelIndex = panels.findIndex((panel) => panel.id === activePanel);
  const childPanels = activePanel === "basis" ? [{ id: "basis", label: "Basis" }]
    : activePanel.startsWith("component:") ? componentSteps.map((item) => ({ id: item.value, label: item.label }))
  ...
  const saveAndContinue = async () => {
    const nodeId = activePanel === "basis" ? "pricing.basis" : activePanel.startsWith("component:") ? "pricing.components" : "pricing.packages";
    const saved = await onProcessAction?.(nodeId);
    if (saved === false) return;
    if (activeChildIndex < childPanels.length - 1) setActiveChild(childPanels[activeChildIndex + 1].id);
    ...
    <div className="commercial-builder">
      <header className="commercial-builder__header">
        <div>
          ...
          <p>
            Enter supplier cost and selling amount in paise. Customer prices are
            calculated by the backend—never typed as a package total.
          </p>
        </div>
        <label>
          Currency
      ...
      <div className="commercial-builder__navigation">
        <WizardSectionNav items={panels} activeId={activePanel} onChange={setActivePanel} ariaLabel="Package pricing sections" />
        <div className="commercial-builder__navigation-actions">
          ...
          {commercial.packages.length < 3 && <Button type="button" primaryClassName="btn" text="+ Package" onClick={addPackage} />}
        </div>
      </div>

      {activePanel === "basis" && <fieldset>
      ...
      {activePanel.startsWith("component:") && <fieldset>
        <legend>Reusable tour components</legend>
        <div className="commercial-builder__stack">
          {commercial.components.map((component, index) => activePanel === `component:${component.componentKey}` && (
            <article
              className="commercial-builder__component"
              key={component.componentKey}
            >
              <WizardSectionNav items={childPanels} activeId={activeChild} onChange={setActiveChild} ariaLabel="Component fields" />
              {activeChild === "details" && <><div className="commercial-builder__row">
                <label>
                  Name
                  <FormInput
              ...
              {activeChild === "pricing" && <><div className="commercial-builder__row">
                <label>
                  Pricing unit
                  <FormSelect
              ...
              <div className="commercial-builder__summary">
                <span>
                  Unit cost {money(component.pricing?.costAmountMinor, commercial.currency)}
                </span>
                <span>
                  Unit sell {money(component.pricing?.sellingAmountMinor, commercial.currency)}
                </span>
              </div></>}
              {activeChild === "upgrade" && <div className="commercial-builder__row">
                <label>
                ...
                <Button
                  ...
                  onClick={() =>
                    update({
                      components: commercial.components.filter(
                        (_, idx) => idx !== index,
                      ),
                      packages: commercial.packages.map((pkg) => ({
                        ...pkg,
                        includedComponentKeys: (
                          pkg.includedComponentKeys || []
                        ).filter((key) => key !== component.componentKey),
                        optionalComponentKeys: (
                          pkg.optionalComponentKeys || []
                        ).filter((key) => key !== component.componentKey),
                      })),
                    })
                  }
        ...
        {!commercial.components.length && <div className="commercial-builder__empty">Add the first reusable cost or service component to continue.</div>}
      </fieldset>}

      {activePanel.startsWith("package:") && <fieldset>
        <legend>Packages (choose 2 or 3)</legend>
        <div className="commercial-builder__packages">
          {commercial.packages.map((pkg, index) => activePanel === `package:${pkg.packageKey}` && (
            <article
              className="commercial-builder__package"
              key={pkg.packageKey}
            >
              <WizardSectionNav items={childPanels} activeId={activeChild} onChange={setActiveChild} ariaLabel="Package fields" />
              {activeChild === "details" && <><div className="commercial-builder__row">
                <label>
                  Tier
                  <FormSelect
              ...
              {activeChild === "assignment" && <div className="commercial-builder__matrix">
                <WizardSectionNav items={commercial.components.map((component, componentIndex) => ({ id: String(componentIndex), label: component.name || `Component ${componentIndex + 1}` }))} activeId={String(activeAssignment)} onChange={(_, index) => setActiveAssignment(index)} ariaLabel="Package component assignments" />
                {commercial.components.map((component, componentIndex) => componentIndex === activeAssignment && (
              ...
              {commercial.packages.length > 2 && (
                <Button
                  ...
                  onClick={() =>
                    update({
                      packages: commercial.packages.filter(
                        (_, idx) => idx !== index,
                      ),
                    })
                  }
      ...
      <footer className="commercial-builder__process-actions">
        <Button type="button" variant="outline" primaryClassName="btn" disabled={(activePanelIndex <= 0 && activeChildIndex <= 0) || processSaving} text="Previous" onClick={goPrevious} />
        <Button type="button" primaryClassName="btn" disabled={processSaving} text={processSaving ? "Saving draft…" : (activeChildIndex < childPanels.length - 1 || activePanelIndex < panels.length - 1) ? "Save & next" : "Save package section"} onClick={saveAndContinue} />
      </footer>
      <p className="commercial-builder__authority">
        Preview values are informational. The saved price, taxes, commission,
        fees, settlement, and margin are recalculated by FinancialEngine.
      </p>
    </div>
  );
}
