...
function DashboardBookingTable({ widget, labels, options, bookingState, bookingQuery, onBookingQueryChange, onViewBooking }) {
    ...
    const optionList = (key, fallback = []) => {
        const opts = tableOptions[key] || fallback;
        return Array.isArray(opts) ? opts.map((item) => (
...
function SettingsForm({ widget, labels, profile, icons = [], onSaveProfile, onSavePassword }) {
    const props = getWidgetProps(widget);
    ...
    const saveProfile = async () => {
        setSaving("profile");
        try {
            const result = await onSaveProfile?.({ name: form.name, avatar: form.avatar });
            if (result?.success) {
                showToast(getLabel(labels, "toastProfileSaved", "Profile updated successfully"));
            } else {
                showToast(result?.message || getLabel(labels, "toastError", "Something went wrong"), "error");
            }
        } catch {
    ...
    const savePassword = async () => {
        ...
        setSaving("password");
        try {
            const result = await onSavePassword?.({ currentPassword: form.currentPassword, newPassword: form.newPassword });
            if (result?.success) {
