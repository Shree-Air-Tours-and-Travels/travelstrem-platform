...
export default function QuickChips({
  ...
  const selectedIds = new Set(
    [...(Array.isArray(activeIds) ? activeIds : []), ...(activeId == null ? [] : [activeId])].map(
      String,
    ),
  );

  const getLabel = (f) => {
    if (f.labelRef && labels[f.labelRef])
      return displayText(labels[f.labelRef], String(f.id || ""));
    return displayText(f.label, String(f.id || ""));
  };

  return (
    <div
      className={`tt-quick-chips ${className}`}
      role="group"
