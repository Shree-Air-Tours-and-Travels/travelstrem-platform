...
export function validateUploadFiles(inputFiles, options = {}) {
  ...
  const maxFiles = Number(
    options.maxFiles ?? (options.multiple === false ? 1 : Number.POSITIVE_INFINITY),
  );
  const errors = [];
  const validFiles = [];
  ...
  if (files.length > maxFiles)
    errors.push({
      code: "too_many_files",
      message: `Choose no more than ${maxFiles} file${maxFiles === 1 ? "" : "s"}.`,
    });
  files.slice(0, maxFiles).forEach((file) => {
    if (accept.some(Boolean) && !accept.some((rule) => acceptedByRule(file, rule))) {
      ...
      errors.push({
        file,
        code: "file_too_large",
        message: `${file.name} exceeds ${formatFileSize(maxFileSize)}.`,
      });
    } else if (Number(file.size) < minFileSize) {
      errors.push({
        file,
        code: "file_too_small",
        message: `${file.name} is empty or below the minimum size.`,
      });
