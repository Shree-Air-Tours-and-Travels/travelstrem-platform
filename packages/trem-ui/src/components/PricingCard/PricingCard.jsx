...
const PricingCard = ({ tour, labels = {}, priceText, cityDisplay, onBook, onContact, onShare, isFavorited, onFavorite }) => {
  if (!tour) return null;
  ...
  return (
    <aside className="pricing-card" aria-label={labels.pricingTitle || "Trip actions"}>
      <div className="pricing-card__header">
        <Button primaryClassName={`pricing-card__fav-toggle${isFav ? " is-fav" : ""}`} variant="text" onClick={() => onFavorite?.(tour)}>
          <Icon name="heart" />
          <span>{isFav ? (labels.saved || "Saved") : (labels.save || "Save")}</span>
        </Button>
        <Button primaryClassName="pricing-card__icon-button" variant="text" isCircular iconLeft="share" onClick={() => onShare(tour)} aria-label="Share tour" />
      </div>
      ...
      <div className="pricing-card__footer">
        <div className="pricing-card__action-grid">
          <Button primaryClassName="pricing-card__button pricing-card__button--primary" variant="solid" color="primary" onClick={() => onBook(tour)}>
            {labels.bookNow || "Book now"}
          </Button>
          <Button primaryClassName="pricing-card__button pricing-card__button--outline" variant="outline" onClick={() => onContact(tour)}>
