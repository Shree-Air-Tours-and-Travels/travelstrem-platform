...
export default function TourFactsView({ tour, labels = {} }) {
  ...
  const facts = [
    { icon: "mapPin", label: hasExplicitRoute ? (labels.route || "Route") : "Destination", value: getCityDisplay(tour) },
    tour.distance != null && Number(tour.distance) > 0
      ? { icon: "mapPin", label: labels.distance || "Distance", value: `${tour.distance} ${labels.km || "km"}` }
      : null,
