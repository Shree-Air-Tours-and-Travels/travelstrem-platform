...
export default function PackagePlans({ tourRef, selectedPackage = "", onSelectPackage }) {
  ...
  const plans = useMemo(() => {
    ...
    return pricing.packages.map((item) => ({
      ...item,
  ...
  return <section className="td-plans" aria-labelledby="td-plans-title">
    <header className="td-plans__header">
      <div>
        <span className="td-plans__eyebrow">{labels.planEyebrow || "Choose your stay and service level"}</span>
        <h2 id="td-plans-title">{labels.planTitle || "Tour packages"}</h2>
        <p>{labels.planDescription || "Compare what is included before requesting your quote."}</p>
      </div>
    </header>
    <div className="td-plans__grid">
      {plans.map((plan) => {
        const selected = selectedPackage === plan.packageKey;
        return <article className={`td-plans__card${plan.recommended ? " is-recommended" : ""}${selected ? " is-selected" : ""}`} key={plan.packageKey}>
          {plan.recommended ? <span className="td-plans__recommended"><Icon name="sparkles" size={14} />{labels.recommended || "Recommended"}</span> : null}
          <div className="td-plans__card-head">
            <div><small>{plan.tier}</small><h3>{plan.name}</h3></div>
            <strong>{plan.priceText}</strong>
          </div>
          {plan.description ? <p className="td-plans__description">{plan.description}</p> : null}
          <div className="td-plans__features">
            {(plan.included || []).slice(0, 8).map((item) => <span key={item}><Icon name="check" size={15} />{item}</span>)}
          </div>
          {(plan.optional || []).length ? <details className="td-plans__optional"><summary>{labels.optionalExtras || "Optional extras"}</summary><p>{plan.optional.join(" · ")}</p></details> : null}
          <Button
            fullWidth
            variant={selected ? "solid" : "outline"}
            color="primary"
            text={selected ? (labels.selectedPlan || "Selected") : (labels.selectPlan || "Choose this plan")}
