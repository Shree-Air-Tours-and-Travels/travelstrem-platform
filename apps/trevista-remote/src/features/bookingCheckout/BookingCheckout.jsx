...
export default function BookingCheckoutPage({ dispatchEvent } = {}) {
  ...
  const referrer = location.state?.from || { label: "Booking Summary", path: `/trevista/bookings/${bookingId}` };
  ...
  const goToSummary = useCallback(() => {
    ...
    navigate(`/trevista/bookings/${bookingId}`);
  ...
  return (
    <main className="booking-checkout-page">
      <div className="booking-summary-page__shell">
        <Breadcrumbs
          items={[
            { label: referrer.label || t("bookingSummary", "Booking Summary"), path: referrer.path || `/trevista/bookings/${bookingId}` },
