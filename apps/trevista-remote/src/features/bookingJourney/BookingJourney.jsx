...
export default function BookingPageContainer({ dispatchEvent } = {}) {
  ...
  const bookingFields = useMemo(() => ({
    ...
    const referrer = useMemo(() => location.state?.from || { label: pageLabels.tours || "Trevista", path: "/trevista" }, [location.state?.from, pageLabels]);
  ...
  const handleSubmit = useCallback(async () => {
    ...
    try {
      ...
      if (typeof dispatchEvent === "function") {
        ...
        navigate(`/trevista/bookings/${bookingId}`, { replace: true, state });
      }
    } catch (err) {
    ...
    <BookingPageView
      ...
      breadcrumbItems={[
        ...(referrer.path !== "/trevista"
          ? [referrer, { label: pageLabels.tours || "Trevista", path: "/trevista" }]
          : [referrer]
        ),
