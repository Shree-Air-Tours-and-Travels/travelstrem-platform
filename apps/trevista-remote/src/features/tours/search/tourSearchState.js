...
const API_SORT_TO_UI = Object.fromEntries(Object.entries(UI_SORT_TO_API).map(([key, value]) => [value, key]));
...
const listParam = (params, key) => [...new Set((params.get(key) || "").split(",").map((value) => value.trim().toLowerCase()).filter(Boolean))];
...
export const parseTourSearchUrl = (search = "") => {
  ...
  return {
    ...
    filters: {
      ...
      duration: { minDays: numberParam(params, "minDays"), maxDays: numberParam(params, "maxDays") },
    ...
    sort: UI_SORT_TO_API[(params.get("sort") || "recommended").toLowerCase()] || TOUR_SORT.RECOMMENDED,
...
export const serializeTourSearchUrl = (state) => {
  ...
  if (state.sort !== TOUR_SORT.RECOMMENDED) set("sort", API_SORT_TO_UI[state.sort] || state.sort.toLowerCase());
...
const facetLabel = (facets, group, value) => facets?.[group]?.find((item) => item.value === value)?.name || value;

export const getActiveTourFilterChips = (state, facets = {}) => {
  const chips = [];
  if (state.query) chips.push({ id: "query", label: `Search: ${state.query}` });
  state.filters.originCityIds.forEach((value) => chips.push({ id: `origin:${value}`, label: `From ${facetLabel(facets, "origins", value)}` }));
  state.filters.destinationCityIds.forEach((value) => chips.push({ id: `destination:${value}`, label: `To ${facetLabel(facets, "destinations", value)}` }));
  state.filters.countryIds.forEach((value) => chips.push({ id: `country:${value}`, label: facetLabel(facets, "countries", value) }));
  state.filters.agencyIds.forEach((value) => chips.push({ id: `agency:${value}`, label: `Agency: ${facetLabel(facets, "agencies", value)}` }));
  state.filters.tagIds.forEach((value) => chips.push({ id: `tag:${value}`, label: facetLabel(facets, "tags", value) }));
  if (state.filters.price.min != null || state.filters.price.max != null) chips.push({ id: "price", label: `Price ${state.filters.price.min ?? 0}–${state.filters.price.max ?? "any"}` });
  if (state.filters.duration.minDays != null || state.filters.duration.maxDays != null) chips.push({ id: "duration", label: `${state.filters.duration.minDays ?? 1}–${state.filters.duration.maxDays ?? "any"} days` });
  if (state.filters.travellers != null) chips.push({ id: "travellers", label: `${state.filters.travellers} travellers` });
  if (state.filters.departureDate) chips.push({ id: "departureDate", label: `Depart ${state.filters.departureDate}` });
  if (state.filters.returnDate) chips.push({ id: "returnDate", label: `Return ${state.filters.returnDate}` });
  if (state.filters.featured != null) chips.push({ id: "featured", label: state.filters.featured ? "Featured" : "Standard" });
  return chips;
};
...
export const removeTourFilter = (state, id) => {
  ...
  else if (type === "origin") next.filters.originCityIds = state.filters.originCityIds.filter((item) => item !== value);
  else if (type === "destination") next.filters.destinationCityIds = state.filters.destinationCityIds.filter((item) => item !== value);
  else if (type === "country") next.filters.countryIds = state.filters.countryIds.filter((item) => item !== value);
  else if (type === "agency") next.filters.agencyIds = state.filters.agencyIds.filter((item) => item !== value);
  else if (type === "tag") next.filters.tagIds = state.filters.tagIds.filter((item) => item !== value);
