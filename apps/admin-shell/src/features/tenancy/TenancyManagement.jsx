...
import {
  ...
  NoDataFound, Pagination, PRODUCT_TYPE, SearchBar, SingleSelect, Spinner, StatusBadge, TextArea,
...
export default function TenancyManagement() {
  ...
  const [form, setForm] = useState({ products: [], agents: [], adminName: "", adminEmail: "", message: "", productName: "", productDescription: "", productStatus: "active", productHidden: false });
  ...
  const resetForm = (record = {}) => setForm({ products: record.productAccess || [], agents: (record.requestedAgents || []).map((agent) => idOf(agent)), adminName: "", adminEmail: "", message: "", productName: record.name || "", productDescription: record.description || "", productStatus: record.status || "active", productHidden: Boolean(record.hidden) });
  ...
  const setCustomTourPartner = (enabled) => act(() => api.patch(`/tenancy/agencies/${idOf(agencyOf(selected))}`, { customTourPartner: enabled }), enabled ? "Custom-tour enquiries will now route to this agency." : "Custom-tour partner removed; new enquiries will fall back to the Master Admin.");
  const inviteAdmin = () => {
  ...
  const decideProduct = (next) => act(() => api.patch(`/tenancy/product-access-requests/${selected._id}`, { status: next, approvedAgentIds: form.agents, decisionNote: form.message }), `Product request ${next}.`);
  ...
  const agencyReview = () => {
    ...
    return <>
      ...
      <section className="tenant-console__review-section"><div className="tenant-console__section-heading"><div><h4>Custom-tour enquiry partner</h4><p>New custom journeys route to the selected agency. Customisations of existing tours always remain with that tour owner.</p></div><StatusBadge value={agency.customTourPartner ? "Selected" : "Not selected"} tone={agency.customTourPartner ? "success" : "neutral"} /></div><Button text={agency.customTourPartner ? "Remove custom-tour partner" : "Select as custom-tour partner"} variant={agency.customTourPartner ? "outline" : "solid"} disabled={!agency.customTourPartner && (agency.status !== "active" || !agency.productAccess?.includes(PRODUCT_TYPE.TREVISTA))} onClick={() => setCustomTourPartner(!agency.customTourPartner)} /></section>
  ...
  const otherReview = () => {
    if (section === "productRequests") {
      const requestedAgents = selected.requestedAgents || [];
      const agentOptions = requestedAgents.map((agent) => ({
        value: idOf(agent),
        label: `${agent.name || "Unknown agent"}${agent.email ? ` · ${agent.email}` : ""}`,
      }));
      return <section className="tenant-console__review-section"><h4>Requested product access</h4><DetailGrid fields={[{ label: "Agency", value: selected.agencyId?.agencyName }, { label: "Current products", value: (selected.currentProducts || []).map(pretty).join(", ") || "None" }, { label: "Requested products", value: (selected.requestedProducts || []).map(pretty).join(", ") }, { label: "Business reason", value: selected.reason }]} />{selected.status === "pending" ? <><MultiSelect label="Agents approved for access" value={form.agents} onChange={(agents) => setForm((value) => ({ ...value, agents }))} options={agentOptions} /><p className="tenant-console__decision-state">Deselect any requested agent who should not receive this product. The agency product is enabled only for the remaining agents.</p><TextArea label="Decision note" value={form.message} onChange={(message) => setForm((value) => ({ ...value, message }))} /><div className="tenant-console__actions"><Button text="Approve access" disabled={requestedAgents.length > 0 && !form.agents.length} onClick={() => decideProduct("approved")} /><Button text="Reject request" color="danger" variant="outline" onClick={() => requireReason(() => decideProduct("rejected"))} /></div></> : null}</section>;
    }
