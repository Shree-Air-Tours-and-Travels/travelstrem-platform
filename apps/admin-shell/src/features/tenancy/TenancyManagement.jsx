...
export default function TenancyManagement() {
  ...
  const open = async (item) => {
    if (section !== "requests" && section !== "agencies") { setSelected(item); if (section === "products") setForm((value) => ({ ...value, productName: item.name || "", productDescription: item.description || "", productStatus: item.status || "active", productHidden: Boolean(item.hidden) })); return; }
    try {
  ...
  const saveProduct = () => act(() => api.put(`/tenancy/products/${selected.key}`, { name: form.productName, description: form.productDescription, status: form.productStatus, hidden: form.productHidden }), "Product updated.");
  ...
  return (
    <section className="tenant-console" aria-label="Multi-tenant administration">
      ...
      {selected && <div className="tenant-console__overlay" role="presentation" onMouseDown={(event) => event.target === event.currentTarget && setSelected(null)}><aside className="tenant-console__drawer" role="dialog" aria-modal="true" aria-label="Record details"><header><div><small>{pretty(section)}</small><h3>{selected.agencyName || selected.agency?.agencyName || selected.name || pretty(selected.action)}</h3></div><button onClick={() => setSelected(null)} aria-label="Close">×</button></header><div className="tenant-console__drawer-body"><dl>{Object.entries(section === "agencies" && selected.agency ? selected.agency : selected).filter(([key, value]) => !["_id", "__v", "documents", "history", "internalNotes"].includes(key) && ["string", "number"].includes(typeof value)).slice(0, 16).map(([key, value]) => <div key={key}><dt>{pretty(key)}</dt><dd>{String(value)}</dd></div>)}</dl>
        ...
        {section === "products" && <div className="tenant-console__convert"><label>Product name<input value={form.productName} onChange={(event) => setForm((value) => ({ ...value, productName: event.target.value }))} /></label><label>Description<textarea value={form.productDescription} onChange={(event) => setForm((value) => ({ ...value, productDescription: event.target.value }))} /></label><label>Status<select value={form.productStatus} onChange={(event) => setForm((value) => ({ ...value, productStatus: event.target.value }))}><option value="active">Active</option><option value="inactive">Inactive</option></select></label><label style={{ display: "flex", alignItems: "center", gap: "0.5rem", cursor: "pointer" }}><input type="checkbox" checked={form.productHidden} onChange={(event) => setForm((value) => ({ ...value, productHidden: event.target.checked }))} /><span>Hidden from customer UI and agency assignment</span></label><Button text="Save product" onClick={saveProduct} /></div>}
      </div></aside></div>}
    </section>
  );
}
