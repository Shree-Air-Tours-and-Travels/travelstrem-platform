...
const RENDERERS = new Set(["dashboard", "trevio", "bookingEngine"]);
...
export const FALLBACK_NAVIGATION_CONFIG = {
  ...
  destinations: [
    { id: "overview", kind: "tab", renderer: "dashboard", tab: "overview", path: "/", activeId: "overview" },
    { id: "bookings", kind: "tab", renderer: "dashboard", tab: "bookings", path: "/", activeId: "bookings" },
    { id: "favorites", kind: "tab", renderer: "dashboard", tab: "favorites", path: "/", activeId: "favorites" },
    { id: "profile", kind: "tab", renderer: "dashboard", tab: "profile", path: "/", activeId: "profile" },
...
const safePatterns = (patterns) => (Array.isArray(patterns) ? patterns : [])
...
export function normalizeNavigationConfig(value = {}) {
  ...
  const destinations = rawDestinations
    ...
    .map((item) => ({
      id: item.id,
      kind: item.kind,
      renderer: RENDERERS.has(item.renderer) ? item.renderer : (item.kind === "remote" ? "" : "dashboard"),
  ...
  const usable = destinations.length ? destinations : FALLBACK_NAVIGATION_CONFIG.destinations;
  ...
  const protocols = Array.isArray(value.security?.allowedExternalProtocols)
    ? value.security.allowedExternalProtocols.filter((protocol) => protocol === "https:" || protocol === "http:")
    : ["https:"];
...
export function resolveDestination(config, location) {
  ...
  const pathMatch = config.destinations.find((item) => item.patterns.some((pattern) => matchesPattern(pathname, pattern)));
  if (pathMatch) return pathMatch;
  const tab = query.get("tab");
  const tabMatch = tab && config.destinations.find((item) => item.tab === tab);
  if (tabMatch) return tabMatch;
  return config.destinations.find((item) => item.id === config.defaultDestination) || config.destinations[0];
}
...
export function resolveNavigationIntent(config, rawIntent, currentOrigin) {
  const intent = typeof rawIntent === "object" && rawIntent !== null
    ? rawIntent
    : { destination: rawIntent };
  ...
  if (destination) {
    return { type: "internal", destination, location: buildDestinationLocation(destination, intent), replace: Boolean(intent.replace) };
  }
  if (target.startsWith("/") && !target.startsWith("//")) {
    return { type: "internal-path", location: { pathname: safePath(target), search: "", hash: "" }, replace: Boolean(intent.replace) };
  }
  try {
    const url = new URL(target, currentOrigin);
    if (url.origin === currentOrigin) {
      return { type: "internal-path", location: { pathname: safePath(url.pathname), search: url.search, hash: url.hash }, replace: Boolean(intent.replace) };
    }
    ...
    if (protocolAllowed && originAllowed && !url.username && !url.password) {
      return { type: "external", url: url.toString(), target: intent.targetWindow === "_blank" ? "_blank" : "_self" };
    }
  } catch {}
