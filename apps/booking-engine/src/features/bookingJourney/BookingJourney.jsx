...
export default function BookingPageContainer({
  dispatchEvent,
  initialTour = null,
  bookingBasePath = "/trevista",
  productLabel = "Trevista",
  productPath = "/trevista",
  tourPath = "",
  tourRef: requestedTourRef = "",
} = {}) {
  const { tourRef: routeTourRef } = useParams();
  const location = useLocation();
  const navigate = useNavigate();

  const decodedRef = decodeURIComponent(String(routeTourRef || requestedTourRef || ""));
  ...
  const [tour, setTour] = useState(location.state?.tour || initialTour);
  const referrer = useMemo(() => location.state?.from || { label: pageLabels.tours || "Trevista", path: bookingBasePath }, [location.state?.from, pageLabels, bookingBasePath]);
  ...
  const handleSubmit = useCallback(async () => {
    ...
    try {
      ...
      if (typeof dispatchEvent === "function") {
        ...
        navigate(`${bookingBasePath}/bookings/${bookingId}`, { replace: true, state });
      }
    } catch (err) {
  ...
  }, [tour, startDate, endDate, adults, children, infants, travelers, contactEmail, contactPhone, user, validateTripStep, validateTravelerStep, dispatchEvent, navigate, pageLabels, bookingBasePath]);
  ...
  return (
    <BookingPageView
      ...
      breadcrumbItems={[
        { label: productLabel, path: productPath },
        ...(tour?.title ? [{ label: tour.title, path: tourPath || undefined }] : []),
