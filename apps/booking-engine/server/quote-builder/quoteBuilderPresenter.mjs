...
const previewRows = (pricing) => {
  ...
  return [
    ...
    { id: "addons", labelRef: "customAddons", amountMinor: composition.addonsMinor || 0 },
    {
      id: "agent",
      labelRef: "agentPrice",
      amountMinor: composition.agentQuoteMinor ?? pricing.baseAmountMinor ?? 0,
    },
    ...
    {
      id: "total",
      labelRef: "customerTotal",
      amountMinor: pricing.finalPayableMinor || 0,
      total: true,
    },
...
const dateSummary = (fields = {}) => {
  const fixed = String(fields.preferredTravelDate || "")
    .split("|")
    .filter(Boolean);
  const dates =
    fixed.length > 1 ? fixed : [fields.preferredStartDate, fields.preferredEndDate].filter(Boolean);
...
const isGenericQuoteMessage = (value) =>
  /^(please\s+)?(give|send|share|provide)(\s+me)?\s+(the\s+|a\s+)?(quote|quotation)(\s+for\s+this\s+tour)?[.!]?$/i.test(
    String(value || "").trim(),
  );
...
const buildRequestSections = (enquiry = {}) => {
  ...
  if (flightRequest === "ADD" || (!flightRequest && fields.flightPreference === "with_flights"))
    extraItems.push({
      id: "flights",
      label: "Add to package · Flights",
      value: "Customer requested flights in addition to the selected package.",
    });
  if (flightRequest === "UNSPECIFIED" || fields.flightPreference === "agent_recommendation")
    extraItems.push({
      id: "flights",
      label: "Flight request",
      value: "Agent recommendation requested.",
    });
  (snapshot.hotels || [])
    .filter((hotel) => hotel.included === false)
    .forEach((hotel, index) => {
      const savedBaseline = (packageBaseline.hotels || []).find(
        (item) => item.stayKey === hotel.stayKey,
      );
  ...
  ((selection.hotelRequests || []).length
    ? selection.hotelRequests
    : snapshot.hotelRequests || []
  ).forEach((request, index) => {
    const budget =
      request.budgetPerNightMinor == null
        ? ""
        : `${request.currency || "INR"} ${(Number(request.budgetPerNightMinor) / 100).toLocaleString("en-IN")} per room / night`;
    extraItems.push({
      id: `hotel-request-${index}`,
      label: `Hotel request · ${request.location || request.stayKey || index + 1}`,
      value:
        [request.propertyClass, request.roomType, budget, request.requirements]
          .filter(Boolean)
          .join(" · ") || "Agent recommendation requested",
    });
  });
  (snapshot.addOns || []).forEach((addOn, index) => {
    extraItems.push({
      id: `addon-${addOn.id || index}`,
      label: `Add to package · ${addOn.title || "Optional add-on"}`,
      value: [
        addOn.description,
        addOn.totalMinor == null
          ? "Agent pricing required"
          : `${snapshot.currency || "INR"} ${(Number(addOn.totalMinor) / 100).toLocaleString("en-IN")}`,
      ]
        .filter(Boolean)
        .join(" · "),
    });
  ...
  Object.entries(enquiry.customizationAnswers || {}).forEach(([question, answer], index) => {
    ...
    if (value)
      extraItems.push({ id: `custom-answer-${index}`, label: readableKey(question), value });
  });
  if ((fields.message || fields.notes) && !isGenericQuoteMessage(fields.message || fields.notes))
    extraItems.push({
      id: "message",
      label: "Additional message",
      value: readableValue(fields.message || fields.notes),
    });
  if (!extraItems.length)
    extraItems.push({
      id: "none",
      label: "Customization",
      value: "No additional request beyond the selected package.",
    });
  return [
    {
      id: "selection",
      ...
      items: [
        { id: "tour", label: "Tour", value: enquiry.tourTitle || "Custom journey" },
        {
          ...
          value: selection.packageName || selection.packageKey || "To be confirmed",
        },
        {
          id: "request-type",
          label: "Request type",
          value:
            snapshot.quoteMode === "CUSTOMIZED" || selection.customizationPreference === "customize"
        ...
        },
        {
          ...
          value:
            packageBaseline.includesFlights === true
              ? `Included${packageBaseline.includedFlightNames?.length ? ` · ${packageBaseline.includedFlightNames.join(", ")}` : ""}`
              : packageBaseline.includesFlights === false
                ? "Not included"
                : "Not captured in this older enquiry",
        },
      ...
      items: [
        {
          id: "travellers",
          label: "Travellers",
          value: fields.travellerCount ? `${fields.travellerCount} traveller(s)` : "Not specified",
        },
...
export function presentQuoteBuilder({
  enquiry,
  persisted = {},
  definition,
  context = {},
  ...
  const tourSections = [
    ...(context.tourSections || fallbackSections.filter((section) => section.id !== "demand")),
  ];
  if (
    (context.source?.inclusions || []).length &&
    !tourSections.some((section) => section.id === "inclusions")
  )
    tourSections.push({
      id: "inclusions",
      titleRef: "sourceInclusions",
      items: context.source.inclusions.map(sectionItemText).filter(Boolean),
    });
  if (
    (context.source?.exclusions || []).length &&
    !tourSections.some((section) => section.id === "exclusions")
  )
    tourSections.push({
      id: "exclusions",
      titleRef: "sourceExclusions",
      items: context.source.exclusions.map(sectionItemText).filter(Boolean),
    });
  ...
  const labelRefs = new Set([
    ...
    ...stage.fields.flatMap((item) => [
      item.labelRef,
      item.emptyTextRef,
      ...(item.options || []).map((option) => option.labelRef),
    ]),
  ...
  const labels = Object.fromEntries(
    [...labelRefs].filter(Boolean).map((ref) => [ref, quoteProcessLabels[ref]]),
  );
  ...
  return {
        ...
        pricing && showPricing
          ? {
              ...
              items: (pricing.quoteItems || []).map((item, index) => ({
                ...
                details: (item.detailRows || []).map((row) => ({
                  label: row.label,
                  value: row.value,
                })),
    ...
    structure: {
      type: "quote-builder",
      header: {
        eyebrowRef: "builderEyebrow",
        titleRef: "builderTitle",
        descriptionRef: "builderDescription",
      },
      process: {
