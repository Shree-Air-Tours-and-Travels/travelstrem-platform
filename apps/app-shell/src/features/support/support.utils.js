...
export const trackSupport = (event, metadata = {}) => {
  if (!Object.values(SUPPORT_ANALYTICS_EVENT).includes(event) || typeof window === "undefined")
    return;
...
export const executeSupportAction = (action, navigate) => {
  ...
  if (
    [
      SUPPORT_ACTION_TYPE.NAVIGATE,
      SUPPORT_ACTION_TYPE.CREATE_TICKET,
      SUPPORT_ACTION_TYPE.CANCELLATION,
      SUPPORT_ACTION_TYPE.REFUND,
      SUPPORT_ACTION_TYPE.RESCHEDULE,
    ].includes(kind)
  ) {
    navigate(target);
    return true;
