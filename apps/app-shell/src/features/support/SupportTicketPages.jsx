...
export function NewSupportRequestPage() {
  ...
  const submit = async () => {
    ...
    try {
      const result = await supportApi.createTicket({ serviceId: params.get("serviceId") || "", categoryId, subject, description });
      setCreated(result.ticket);
      trackSupport(SUPPORT_ANALYTICS_EVENT.TICKET_CREATED, { ticketId: result.ticket?.id, categoryId });
  ...
  return <SupportLayout title="Create support request" subtitle={reviewing ? "Review the details before submitting." : "Tell us what you need help with."}><ResourceBoundary {...categories}>
...
export function SupportRequestsPage() {
  ...
  return <SupportLayout title="My Support Requests" subtitle="Track requests and replies from the support team." actions={<Button text="New request" onClick={() => { trackSupport(SUPPORT_ANALYTICS_EVENT.TICKET_STARTED); navigate("/help/new-request"); }} />}><ResourceBoundary {...resource}>
    ...
    {resource.data?.tickets?.length ? <div className="support-list">{resource.data.tickets.map((ticket) => <SupportTicketCard key={ticket.id} ticket={{ ...ticket, status: { id: ticket.status, label: labelByStatus[ticket.status] }, updatedLabel: `Updated ${formatDateTime(ticket.lastActivityAt)}` }} onSelect={() => navigate(`/help/requests/${ticket.id}`)} />)}</div> : <EmptyState {...resource.data?.emptyState} />}
  </ResourceBoundary></SupportLayout>;
}
...
export function SupportTicketDetailPage() {
  ...
  return <SupportLayout title={ticket?.subject || "Support request"} subtitle={ticket?.reference}><ResourceBoundary {...resource}>{ticket ? <>
    <div className="support-ticket-summary"><div><span>Status</span><strong>{resource.data?.status?.label}</strong></div><div><span>Category</span><strong>{resource.data?.category?.label}</strong></div><div><span>Created</span><strong>{formatDateTime(ticket.createdAt)}</strong></div></div>
