...
export default function FavoritesView({ favorites, loading, onRemoveFavorite, onViewFavorite }) {
  ...
  const filtered =
    productFilter === "all"
      ? favorites
      : favorites?.filter(
          (f) => f.product === productFilter || (!f.product && productFilter === "trevista"),
        );
    ...
    <div className="dfv">
      <div className="dfv__header">
        <div>
          ...
          <p className="dfv__subtitle">
            {filtered?.length || 0} saved trip{(filtered?.length || 0) !== 1 ? "s" : ""}
          </p>
        </div>
        <Dropdown
          hoverable={false}
        ...
        <EmptyState
          ...
          description={
            productFilter !== "all"
              ...
              : "Save trips you love and they'll appear here."
          }
        />
      )}
    </div>
