...
export const FALLBACK_NAVIGATION_CONFIG = {
  ...
  destinations: [
    { id: "overview", kind: "tab", renderer: "app-shell", tab: "overview", path: "/", activeId: "overview" },
...
export function normalizeNavigationConfig(value = {}) {
  ...
  const destinations = rawDestinations
    ...
    .map((item) => ({
      id: item.id,
      kind: item.kind,
      renderer: RENDERERS.has(item.renderer) ? item.renderer : (item.kind === "remote" ? "" : "app-shell"),
  ...
  const usable = destinations.length ? destinations : normalizeNavigationConfig(FALLBACK_NAVIGATION_CONFIG).destinations;
