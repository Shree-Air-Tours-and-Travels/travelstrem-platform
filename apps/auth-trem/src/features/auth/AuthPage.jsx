...
const clientPortalOrigins = (portal, fallbackUrl) => {
  ...
  return new Set([fallbackUrl.origin, ...configured.flatMap((entry) => {
    try { return entry ? [new URL(entry).origin] : []; } catch { return []; }
  })]);
};
...
const GoogleMark = () => (
  <svg className="auth-trem__google-mark" viewBox="0 0 24 24" aria-hidden="true">
    ...
    <path fill="#FBBC05" d="M6.4 13.7A6 6 0 0 1 6.1 12c0-.6.1-1.2.3-1.7V7.4H3A10 10 0 0 0 3 16.6l3.4-2.9Z" />
    <path fill="#EA4335" d="M12 6.2c1.5 0 2.8.5 3.9 1.5l2.9-2.9A9.8 9.8 0 0 0 3 7.4l3.4 2.9C7.2 8 9.4 6.2 12 6.2Z" />
  </svg>
);
...
export default function AuthPage({
  ...
  const masterAdminEmail = String(process.env.REACT_APP_MASTER_ADMIN_EMAIL || "").trim().toLowerCase();
  const [methods, setMethods] = useState({ google: { enabled: false }, mobile: { enabled: true, available: false } });
  ...
  const [emailForm, setEmailForm] = useState({ name: "", email: "", phone: "", password: "", confirmPassword: "", adminOtp: "", adminPin: "" });
  ...
  const requestOtp = async (event) => {
    ...
    try {
      ...
      setError(data.message || "Mobile verification is coming soon. Please continue with Google for now.");
    } finally {
      setLoading(false);
    }
  };
  ...
  const verifyOtp = async (event) => {
    ...
    try {
      ...
      setError(verifyError?.response?.data?.message || "The verification code is invalid or expired.");
    } finally {
      setLoading(false);
    }
  };
  ...
  const submitEmailAuth = async (event) => {
    ...
    try {
      if (emailMode === "register") {
        ...
        if (isMasterAdminRegistration && adminRegistration?.status !== "verified") throw new Error("Verify the administrator registration code first.");
        if (isMasterAdminRegistration && !/^\d{6}$/.test(emailForm.adminPin)) throw new Error("Enter the configured 6 digit Admin PIN.");
        ...
        if (data?.status === "pending_approval") {
          setEmailMode("login");
          setError(data.message || "Registration submitted. Sign in after your account is approved.");
          return;
        }
        redirectAfterAuth();
        return;
      ...
      const response = await authService.login({ email: emailForm.email.trim(), password: emailForm.password });
      const data = response?.data || response;
  ...
  const requestAdminRegistrationOtp = async () => {
    ...
    try {
      ...
      setError(registrationError?.response?.data?.message || "Unable to send the registration code.");
    } finally {
      setLoading(false);
    }
  };
  ...
  const verifyAdminRegistrationOtp = async () => {
    ...
    try {
      ...
      setError(registrationError?.response?.data?.message || "Unable to verify the registration code.");
    } finally {
      setLoading(false);
    }
  };
  ...
  const requestPasswordReset = async () => {
    ...
    try {
      ...
      setError((response?.data || response)?.message || "If that email is registered, a password reset code has been sent.");
  ...
  const resendEmailLoginOtp = async () => {
    ...
    setError("");
    try {
      const response = await authService.resendLoginOtp({ verificationId: emailOtpStep?.verificationId });
  ...
  return (
    <>
      <AuthHeader
        config={{ brand: { name: "TravelsTREM", subtitle: "Tours · Reservations · Experiences · Management" } }}
        theme={theme}
      ...
      <main className="auth-trem">
        <div className="auth-trem__experience">
          <section className="auth-trem__company">
            ...
            <h1 className="auth-trem__company-title">One account for<br />every journey.</h1>
            <p className="auth-trem__company-description">Discover curated trips, manage reservations and keep every travel detail together with <strong className="auth-trem__company-description-highlight">TravelsTREM.</strong></p>
            <div className="auth-trem__highlights">
              <article className="auth-trem__highlight"><span className="auth-trem__highlight-icon"><Icon name="map" size={20} /></span><span><strong>Curated experiences</strong><small>Thoughtfully planned adventures and holiday packages.</small></span></article>
              <article className="auth-trem__highlight"><span className="auth-trem__highlight-icon"><Icon name="calendar" size={20} /></span><span><strong>Reservations in one place</strong><small>Keep bookings, payments and travel details together.</small></span></article>
              <article className="auth-trem__highlight"><span className="auth-trem__highlight-icon"><Icon name="support" size={20} /></span><span><strong>Travel support</strong><small>Helpful guidance from our travel team.</small></span></article>
            </div>
          </section>
            ...
            <div className="auth-trem__card">
              ...
              {screen === "methods" && isCustomer && !activationToken ? (
                <div className="auth-trem__method-list">
                  ...
                  <Button variant="outline" iconLeft="phoneCall" text="Continue with Mobile Number" onClick={() => { setError(""); setScreen("mobile"); }} primaryClassName="auth-trem__mobile-button" />
                  <div className="auth-trem__guest-divider"><span>or use email</span></div>
                </div>
              ) : null}
                ...
                <form className="auth-trem__form auth-trem__email-form" onSubmit={submitEmailAuth}>
                  {registerEnabled ? (
                    <div className="auth-trem__tabs">
                      <Button variant="text" type="button" text="Log In" onClick={() => { setError(""); setEmailMode("login"); }} primaryClassName={`auth-trem__tab ${emailMode === "login" ? "is-active" : ""}`} />
                      <Button variant="text" type="button" text="Register" onClick={() => { setError(""); setEmailMode("register"); }} primaryClassName={`auth-trem__tab ${emailMode === "register" ? "is-active" : ""}`} />
                    </div>
                  ) : null}
                  {emailMode === "register" ? <input className="auth-trem__field" autoComplete="name" placeholder="Full name" value={emailForm.name} onChange={(event) => setEmailForm((value) => ({ ...value, name: event.target.value }))} required /> : null}
                  <input className="auth-trem__field" type="email" inputMode="email" autoComplete="email" placeholder="Email" value={emailForm.email} onChange={(event) => { const email = event.target.value; setEmailForm((value) => ({ ...value, email, adminOtp: "", adminPin: "" })); setAdminRegistration(null); }} required />
                  {isAdmin && emailMode === "register" ? <input className="auth-trem__field" type="tel" inputMode="tel" autoComplete="tel" placeholder="Mobile number" value={emailForm.phone} onChange={(event) => { const phone = event.target.value; setEmailForm((value) => ({ ...value, phone, adminOtp: "", adminPin: "" })); setAdminRegistration(null); }} required /> : null}
                  <SecretField autoComplete={emailMode === "register" ? "new-password" : "current-password"} placeholder="Password" value={emailForm.password} onChange={(event) => setEmailForm((value) => ({ ...value, password: event.target.value }))} required />
                  {emailMode === "register" ? <SecretField autoComplete="new-password" placeholder="Confirm password" value={emailForm.confirmPassword} onChange={(event) => setEmailForm((value) => ({ ...value, confirmPassword: event.target.value }))} required /> : null}
                  {isMasterAdminRegistration ? (
                    <div className="auth-trem__secret">
                      {adminRegistration?.status !== "verified" ? (
                        <div className="auth-trem__otp-row">
                          <input className="auth-trem__field" inputMode="numeric" autoComplete="one-time-code" placeholder="Registration code" pattern="[0-9]{6}" maxLength={6} value={emailForm.adminOtp} onChange={(event) => setEmailForm((value) => ({ ...value, adminOtp: event.target.value.replace(/\D/g, "").slice(0, 6) }))} />
                          <Button variant="outline" size="small" type="button" text={adminRegistration?.status === "otp_sent" ? "Verify code" : "Send code"} disabled={loading} onClick={adminRegistration?.status === "otp_sent" ? verifyAdminRegistrationOtp : requestAdminRegistrationOtp} />
                        </div>
                      ) : (
                        <SecretField inputMode="numeric" autoComplete="off" placeholder="6 digit Admin PIN" pattern="[0-9]{6}" maxLength={6} value={emailForm.adminPin} onChange={(event) => setEmailForm((value) => ({ ...value, adminPin: event.target.value.replace(/\D/g, "").slice(0, 6) }))} required />
                      )}
                      <div className="auth-trem__hint">The first master administrator must verify the configured email and mobile before entering the private Admin PIN.</div>
                    </div>
                  ) : null}
                  {emailMode === "login" ? <Button variant="text" type="button" text="Forgot password?" onClick={requestPasswordReset} primaryClassName="auth-trem__forgot-link" /> : null}
                  <Button variant="solid" color="primary" type="submit" text={loading ? "Please wait…" : emailMode === "register" ? "Register" : "Log In"} disabled={loading} primaryClassName="auth-trem__primary" />
                  {isCustomer ? <Button variant="text" type="button" text="Continue as guest" onClick={continueAsGuest} primaryClassName="auth-trem__guest-link" /> : null}
                </form>
              ) : null}

              {screen === "mobile" ? (
                <form className="auth-trem__form" onSubmit={requestOtp}>
                  <label className="auth-trem__mobile-label" htmlFor="mobile-number">Mobile number</label>
                  <div className="auth-trem__mobile-input">
                    <span>India&nbsp; +91</span>
                    <input id="mobile-number" type="tel" inputMode="tel" autoComplete="tel" placeholder="98765 43210" value={phoneNumber} onChange={(event) => setPhoneNumber(event.target.value)} required autoFocus />
                  </div>
                  <Button variant="solid" color="primary" type="submit" text={loading ? "Requesting code…" : "Continue"} disabled={loading} primaryClassName="auth-trem__primary" />
                  <Button variant="text" type="button" text="Back to sign in options" onClick={() => { setError(""); setScreen("methods"); }} primaryClassName="auth-trem__guest-link" />
                </form>
              ) : null}

              {screen === "otp" ? (
                <form className="auth-trem__form" onSubmit={verifyOtp}>
                  <label className="auth-trem__mobile-label" htmlFor="mobile-otp">6-digit verification code</label>
                  <input ref={otpRef} id="mobile-otp" className="auth-trem__field auth-trem__field--otp" inputMode="numeric" autoComplete="one-time-code" pattern="[0-9]{6}" maxLength={6} value={otp} onChange={(event) => setOtp(event.target.value.replace(/\D/g, "").slice(0, 6))} required />
                  <Button variant="solid" color="primary" type="submit" text={loading ? "Verifying…" : "Verify"} disabled={loading || otp.length !== 6} primaryClassName="auth-trem__primary" />
                  {challenge?.developmentOtp ? <p className="auth-trem__dev-otp">Development code: <strong>{challenge.developmentOtp}</strong></p> : null}
                  <div className="auth-trem__otp-actions">
                    <Button variant="text" type="button" text={cooldown ? `Resend in ${cooldown}s` : "Resend OTP"} disabled={loading || cooldown > 0} onClick={requestOtp} />
                    <Button variant="text" type="button" text="Change number" onClick={() => { setOtp(""); setError(""); setScreen("mobile"); }} />
                  </div>
                </form>
              ) : null}
              ...
              {screen === "emailOtp" ? (
                <form className="auth-trem__form" onSubmit={verifyEmailOtp}>
                  <label className="auth-trem__mobile-label" htmlFor="email-login-otp">6-digit verification code</label>
                  <input ref={otpRef} id="email-login-otp" className="auth-trem__field auth-trem__field--otp" inputMode="numeric" autoComplete="one-time-code" pattern="[0-9]{6}" maxLength={6} value={otp} onChange={(event) => setOtp(event.target.value.replace(/\D/g, "").slice(0, 6))} required />
                  <Button variant="solid" color="primary" type="submit" text={loading ? "Verifying…" : "Verify and continue"} disabled={loading || otp.length !== 6} primaryClassName="auth-trem__primary" />
                  <Button variant="text" type="button" text="Resend code" disabled={loading} onClick={resendEmailLoginOtp} />
                  <Button variant="text" type="button" text="Back to login" onClick={() => { setOtp(""); setError(""); setScreen(isCustomer ? "methods" : "email"); }} primaryClassName="auth-trem__guest-link" />
                </form>
              ) : null}

              {screen === "resetPassword" ? (
                <form className="auth-trem__form" onSubmit={submitPasswordReset}>
                  <label className="auth-trem__mobile-label" htmlFor="password-reset-otp">6-digit reset code</label>
                  <input id="password-reset-otp" className="auth-trem__field auth-trem__field--otp" inputMode="numeric" autoComplete="one-time-code" pattern="[0-9]{6}" maxLength={6} value={resetForm.otp} onChange={(event) => setResetForm((value) => ({ ...value, otp: event.target.value.replace(/\D/g, "").slice(0, 6) }))} required autoFocus />
                  <SecretField autoComplete="new-password" placeholder="New password" value={resetForm.password} onChange={(event) => setResetForm((value) => ({ ...value, password: event.target.value }))} required minLength={8} />
              ...
              {accessRequest?.href && !isAdmin ? <div className="auth-trem__access-request"><span>{accessRequest.prompt}</span><a href={accessRequest.href}>{accessRequest.label}</a></div> : null}
            </div>
          </section>
        </div>
      </main>
