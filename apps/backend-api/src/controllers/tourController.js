...
import { readStaticPayload } from "../services/staticPayload.js";
import {
    getHandlerFromReq,
    filterRoleActionsForHandler,
    enrichTourWithStatic,
...
const buildComponentData = (payload, handler, dataTitle = "Tours", toursArray = []) => {
    const actions = filterRoleActionsForHandler(payload?.actions || {}, handler);
    return {
        state: {
            data: {
                id: null,
                title: payload?.state?.title || dataTitle,
                description: payload?.state?.description || "",
                tours: toursArray,
            },
        },
        structure: payload?.structure || null,
        config: payload?.config || { header: { title: "Back to Tours" } },
        actions,
    };
};
...
export const getTours = async (req, res) => {
    ...
    try {
        ...
        const tours = (Array.isArray(toursRaw) ? toursRaw : []).map((doc) => {
            ...
            const enriched = enrichTourWithStatic(tourObj, payload);
            const normalized = normalizeTourForResponse(enriched, priceInfo);
            return normalized;
        });

        const componentData = buildComponentData(payload, handler, "Our Popular Tour Packages", tours);
        ...
        const componentData = buildComponentData(payload, handler, "Our Popular Tour Packages", []);
...
export const getTourById = async (req, res) => {
    ...
    try {
        ...
        if (!tourRaw) {
            const componentData = buildComponentData(payload, handler, "Tour Not Found", []);
        ...
        const normalized = normalizeTourForResponse(enrichedTour, priceInfo);

        const componentData = buildComponentData(payload, handler, normalized.title || "Tour Details", [normalized]);
...
export const createTour = async (req, res) => {
    ...
    try {
        ...
        const newTour = new Tour(sanitized);
        const savedTour = await newTour.save();

        const payload = await readStaticPayload();
        const priceInfo = buildPriceInfo(savedTour, new Date());
        const enriched = enrichTourWithStatic(savedTour.toObject ? savedTour.toObject() : savedTour, payload);
        const normalized = normalizeTourForResponse(enriched, priceInfo);

        const componentData = buildComponentData(payload, handler, "Tour Created", [normalized]);
        ...
        const payload = await readStaticPayload();
        const componentData = buildComponentData(payload, handler, "Create Tour", []);
...
export const updateTour = async (req, res) => {
    ...
    try {
        ...
        const updatedTour = await Tour.findByIdAndUpdate(id, sanitized, {
            new: true,
            runValidators: true,
        });
        ...
        if (!updatedTour) {
            const payload = await readStaticPayload();
            const componentData = buildComponentData(payload, handler, "Update Tour", []);
            return sendJson(res, 404, {
                status: "error",
                message: "Tour not found",
                handler,
        ...
        const enriched = enrichTourWithStatic(updatedTour.toObject ? updatedTour.toObject() : updatedTour, payload);
        const normalized = normalizeTourForResponse(enriched, priceInfo);

        const componentData = buildComponentData(payload, handler, "Tour Updated", [normalized]);
...
export const deleteTour = async (req, res) => {
    ...
    try {
        ...
        const payload = await readStaticPayload();
        if (!deletedTour) {
            const componentData = buildComponentData(payload, handler, "Delete Tour", []);
            return sendJson(res, 404, {
                status: "error",
                message: "Tour not found",
                handler,
        ...
        const componentData = buildComponentData(payload, handler, "Tour Deleted", []);
        return sendJson(res, 200, {
            status: "success",
...
export const deleteAllTours = async (req, res) => {
    ...
    try {
        ...
        const componentData = buildComponentData(payload, handler, "Our Popular Tour Packages deleted", []);
        return sendJson(res, 200, {
            status: "success",
        ...
        const payload = await readStaticPayload();
        const componentData = buildComponentData(payload, handler, "Delete Our Popular Tour Packages", []);
        return sendJson(res, 500, {
            status: "error",
