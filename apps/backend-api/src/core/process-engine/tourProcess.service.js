...
function assertCanEdit(req, tour) {
  ...
  if (id(tour.agencyId) !== id(req.access?.agencyId || req.user?.agencyId)) throw Object.assign(new Error("You cannot edit this draft"), { status: 403 });
  if ((req.access?.role || req.user?.agencyRole) !== "partner_admin" && id(tour.ownerAgent) !== actorId) throw Object.assign(new Error("You cannot edit this draft"), { status: 403 });
}
...
export async function submitTourProcessAction(req, { tourId, nodeId, payload }) {
  ...
  const nextTransition = applyProcessAction(TOUR_BUILDER_PROCESS, persisted, { nodeId, data: payload });
  ...
  if (!tour) {
    ...
    const isAgencyActor = req.user?.role === "agent" || ["partner_agent", "partner_admin"].includes(req.access?.role || req.user?.agencyRole);
    tour = new Tour({
      ...sanitized,
