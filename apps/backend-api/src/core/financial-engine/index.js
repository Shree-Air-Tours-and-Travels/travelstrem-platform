...
import { calculateSettlementFromFinancials, createSettlementRecord } from "./services/settlement.service.js";
...
import { calculateTourCommercials, calculateTourCustomizationPreview } from "./services/tour-commercial.service.js";
...
const resolve = (input) => input.config ? Promise.resolve(validateFinancialConfig(mergeConfig(DEFAULT_FINANCIAL_CONFIG, input.config))) : resolveFinancialConfig(input.context || input, dependencies.repositories);
...
export const FinancialEngine = Object.freeze({
  ...
  async calculateBookingFinancials(input = {}) {
    if (input.tour?.commercial?.version === "COMPONENTS_V1") {
      ...
      const financials = calculateFinancials({ agentAmountMinor: commercial.agentAmountMinor, currency: commercial.currency, config });
      return { ...financials, commercial };
    }
    const config = await resolve(input);
    return calculateFinancials({ agentAmountMinor: input.agentAmountMinor, currency: input.currency || config.currency, config });
  },
  async calculateQuote(input = {}) {
    return calculateQuoteWithConfig(input, dependencies);
  },
  async createQuote(input = {}) {
    const calculated = input.financialSnapshot ? { config: input.configSnapshot, financials: input.financialSnapshot, pricing: input.pricingSnapshot } : await calculateQuoteWithConfig(input, dependencies);
    return createQuoteRecord({ ...input, configSnapshot: calculated.config, financialSnapshot: calculated.financials, pricingSnapshot: calculated.pricing }, dependencies.repositories);
  },
  async createPayment(input = {}) { return createPaymentRecord(input, { ...dependencies }); },
  async processPayment(input = {}) {
    ...
    if (financials && dependencies.repositories.settlements?.createIdempotent) {
      const settlement = calculateSettlementFromFinancials({ financials, paidMinor: payment.amountMinor ?? input.amountMinor ?? financials.customer.payableMinor, refundedMinor: 0 });
      await createSettlementRecord({ bookingId: input.bookingId || payment.bookingId, paymentId: payment.id || payment._id, agencyId: input.agencyId || payment.agencyId, idempotencyKey: `payment:${payment.id || payment._id}:settlement`, settlement, financialSnapshot: financials }, dependencies.repositories);
    }
    return payment;
  },
  async calculateSettlement(input = {}) { return calculateSettlementFromFinancials(input); },
  ...
  async resolveConfig(input = {}) { return resolveFinancialConfig(input, dependencies.repositories); },
  ledger: Object.freeze({
    record(entry) { return ledger.record(entry); },
  }),
});
