...
export function assertMinor(value, name = "amountMinor", { allowNegative = false } = {}) {
    if (!Number.isSafeInteger(value))
        throw new TypeError(`${name} must be a safe integer in paise`);
...
export function rupeesToMinor(value, name = "amount") {
    const raw = String(value ?? "0")
        ...
        .replace(/,/g, "");
    if (!/^-?\d+(?:\.\d{1,2})?$/.test(raw))
        throw new TypeError(`${name} must be a monetary value with at most two decimals`);
...
export function sumMinor(values, name = "amounts") {
    const total = values.reduce(
        (sum, value, index) =>
            sum + BigInt(assertMinor(value, `${name}[${index}]`, { allowNegative: true })),
        0n,
    );
    const result = Number(total);
    if (!Number.isSafeInteger(result))
        throw new RangeError(`${name} total exceeds the supported monetary range`);
    return result;
}
...
export function percentageOf(amountMinor, rateBasisPoints) {
    ...
    if (!Number.isSafeInteger(rateBasisPoints) || rateBasisPoints < 0)
        throw new TypeError("rateBasisPoints must be a non-negative integer");
...
export function percentToBasisPoints(value, name = "percentage") {
    ...
    if (!/^\d+(?:\.\d{1,2})?$/.test(raw))
        throw new TypeError(`${name} must have at most two decimal places`);
