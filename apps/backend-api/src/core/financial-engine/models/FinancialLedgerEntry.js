...
schema.pre("findOneAndUpdate", function immutableUpsert() {
    ...
    if (!this.getOptions().upsert || Object.keys(update).some((key) => key !== "$setOnInsert"))
        throw new Error("Financial ledger entries are immutable");
});
schema.pre("save", function immutableSave() {
    if (!this.isNew) throw new Error("Financial ledger entries are immutable");
});
export default mongoose.models?.FinancialLedgerEntry ||
    mongoose.model("FinancialLedgerEntry", schema);
