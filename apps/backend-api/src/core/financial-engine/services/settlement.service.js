...
export function calculateSettlementFromFinancials({
    financials,
    paidMinor = financials?.customer?.payableMinor,
    refundedMinor = 0,
}) {
    ...
    const prorate = (amount) =>
        expectedMinor
            ? Number(
                  (BigInt(amount) * ratioNumerator + BigInt(expectedMinor / 2)) /
...
export async function createSettlementRecord(input, repositories) {
    if (!repositories.settlements?.createIdempotent)
        throw new Error("Settlement repository is not configured");
