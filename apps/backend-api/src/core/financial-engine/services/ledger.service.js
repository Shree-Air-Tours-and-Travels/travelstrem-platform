...
function validate(entry) {
  ...
  if (!["CREDIT", "DEBIT"].includes(entry.direction)) throw new TypeError("Ledger direction must be CREDIT or DEBIT");
...
export function createLedgerService(repositories) {
  return Object.freeze({
    async record(entry) {
      if (!repositories.ledger?.append) throw new Error("Ledger repository is not configured");
      return repositories.ledger.append(validate(entry));
    },
    async recordMany(entries) {
      if (!repositories.ledger?.appendMany) throw new Error("Ledger repository is not configured");
