...
export async function createPaymentRecord(input, { repositories, providers }) {
  if (!repositories.payments?.createPending) throw new Error("Payment repository is not configured");
  if (!input.financialSnapshot || !input.configSnapshot) throw new TypeError("Payment config and financial snapshots are required");
  const idempotencyKey = requireIdempotencyKey(input.idempotencyKey);
  const existing = await repositories.payments.findByIdempotencyKey?.(idempotencyKey);
  if (existing) return existing;
  const amountMinor = assertMinor(input.amountMinor ?? input.financialSnapshot?.customer?.payableMinor, "amountMinor");
  const providerName = String(input.provider || "manual").toLowerCase();
  const provider = providers[providerName];
  if (providerName !== "manual" && !provider) throw new Error(`Payment provider '${providerName}' is not configured`);
  const providerPayment = provider ? await provider.createPayment({ amountMinor, currency: input.currency || input.financialSnapshot.customer.currency, reference: input.reference || idempotencyKey, metadata: input.metadata }) : null;
  return repositories.payments.createPending({ ...input, amountMinor, idempotencyKey, provider: providerName, providerPaymentId: providerPayment?.id || input.providerPaymentId || "", providerPayload: providerPayment });
}

export async function processPaymentRecord(input, { repositories, providers, ledger }) {
  requireIdempotencyKey(input.idempotencyKey);
  if (!repositories.payments?.markProcessed) throw new Error("Payment repository is not configured");
  if (input.provider && providers[input.provider] && input.rawBody != null) {
    const valid = await providers[input.provider].verifyWebhook({ rawBody: input.rawBody, signature: input.signature });
    if (!valid) throw new Error("Invalid payment webhook signature");
  }
  let payment = await repositories.payments.findProcessedByEvent?.(input.idempotencyKey)
  ...
  const prorate = (amountMinor) => expectedMinor ? Number((BigInt(amountMinor) * BigInt(Math.min(capturedMinor, expectedMinor)) + BigInt(Math.floor(expectedMinor / 2))) / BigInt(expectedMinor)) : 0;
  const entries = [
  ...
  ].filter(([, , amountMinor]) => amountMinor > 0).map(([type, direction, amountMinor]) => ({ type, direction, amountMinor, currency, bookingId, paymentId, idempotencyKey: `payment:${paymentId}:ledger:${type}` }));
