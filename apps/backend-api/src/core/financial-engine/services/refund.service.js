...
export function calculateRefundAmount({
    ...
    const requestedMinor =
        amountMinor == null
            ? percentageOf(paidMinor, rateBasisPoints ?? 10000)
            : assertMinor(amountMinor, "amountMinor");
    const refundableRemainingMinor = Math.max(0, paidMinor - alreadyRefundedMinor);
    if (requestedMinor > refundableRemainingMinor)
        throw new RangeError("Refund exceeds the refundable payment balance");
    ...
    return {
        ...
        kind:
            requestedMinor === refundableRemainingMinor && alreadyRefundedMinor === 0
                ? "FULL"
                : "PARTIAL",
        reversals: {
            commissionMinor: ratio(
                financials.platform.commissionMinor,
                config.refund.commissionRefundable,
            ),
            platformGstMinor: ratio(
                financials.platform.gstMinor,
                config.refund.platformGstRefundable,
            ),
            gatewayFeeMinor: ratio(
                financials.gateway.totalMinor,
                config.refund.gatewayFeeRefundable,
            ),
...
export async function processRefundRecord(input, { repositories, providers, ledger }) {
    if (!repositories.refunds?.createPending || !repositories.refunds?.markProcessed)
        throw new Error("Refund repository is not configured");
    ...
    await ledger.recordMany(
        [
            ...
            ...Object.entries(input.refund.reversals)
                .filter(([, amount]) => amount > 0)
                .map(([component, amountMinor]) => ({
                    type: "REVERSAL",
                    direction: "DEBIT",
                    amountMinor,
                    metadata: { component },
            ...
            refundId: processed.id || processed._id,
            currency: input.refund.currency,
            idempotencyKey: `refund:${processed.id || processed._id}:ledger:${index}`,
        })),
    );
    if (repositories.settlements?.adjustForRefund)
        await repositories.settlements.adjustForRefund({
            bookingId: input.bookingId,
            refundId: processed.id || processed._id,
            refund: input.refund,
        });
