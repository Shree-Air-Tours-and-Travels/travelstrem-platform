const PRICING_UNITS = new Set([
  "FIXED", "PER_BOOKING", "PER_PERSON", "PER_ADULT", "PER_CHILD", "PER_INFANT",
  "PER_ROOM", "PER_NIGHT", "PER_ROOM_PER_NIGHT", "PER_PERSON_PER_NIGHT",
  "PER_VEHICLE", "PER_TRIP", "PER_DAY", "PER_GROUP",
]);
...
const quantityFor = (unit, basis) => {
  ...
  switch (unit) {
    case "PER_PERSON": return travellers;
    case "PER_ADULT": return basis.adults;
    case "PER_CHILD": return basis.children;
    case "PER_INFANT": return basis.infants;
    case "PER_ROOM": return basis.rooms;
    case "PER_NIGHT": return basis.nights;
    case "PER_ROOM_PER_NIGHT": return multiply(basis.rooms, basis.nights, "Room-night quantity");
    case "PER_PERSON_PER_NIGHT": return multiply(travellers, basis.nights, "Person-night quantity");
    case "PER_VEHICLE": return basis.vehicles;
    case "PER_DAY": return basis.days;
    case "PER_GROUP":
    case "PER_TRIP":
    case "PER_BOOKING":
    case "FIXED": return 1;
    default: throw new Error(`Unsupported pricing unit: ${unit}`);
  }
};

const normalizeBasis = (input = {}, tour = {}) => ({
  adults: integer(input.adults ?? input.travellers?.adults ?? input.adultCount ?? 1, "Adults"),
  children: integer(input.children ?? input.travellers?.children ?? input.childCount ?? 0, "Children"),
  infants: integer(input.infants ?? input.travellers?.infants ?? input.infantCount ?? 0, "Infants"),
...
const componentAmount = (component, basis, moneyField) => {
  ...
  const unitAmountMinor = integer(pricing[moneyField] ?? 0, `${component.name || component.componentKey} ${moneyField}`);
  const quantity = quantityFor(unit, basis);
  return { unit, quantity, unitAmountMinor, amountMinor: multiply(unitAmountMinor, quantity, "Component amount") };
};

const percentageAmount = (amountMinor, percent, name) => {
  const basisPoints = Math.round(Number(percent || 0) * 100);
  if (!Number.isSafeInteger(basisPoints) || basisPoints < 0 || basisPoints > 10000) throw new TypeError(`${name} must be between 0 and 100 percent`);
...
export function calculateTourCommercials({ tour, packageKey, selections = {} } = {}) {
  ...
  if (commercial.version !== "COMPONENTS_V1") throw new Error("Tour does not use component pricing");
  const packages = Array.isArray(commercial.packages) ? commercial.packages : [];
  const selectedPackage = packages.find((item) => item.enabled !== false && (item.packageKey === packageKey || item.tier === packageKey));
  if (!selectedPackage) throw new Error("A valid enabled package is required");
  const components = new Map((commercial.components || []).filter((item) => item.active !== false).map((item) => [String(item.componentKey), item]));
  ...
  const requestedOptionalKeys = uniqueKeys(selections.optionalComponentKeys || selections.addonKeys || []);
  for (const key of requestedOptionalKeys) if (!allowedOptionalKeys.has(key)) throw new Error(`Component '${key}' is not optional for this package`);
  const basis = normalizeBasis(selections, tour);
  if (basis.adults + basis.children + basis.infants < 1) throw new Error("At least one traveller is required");
  ...
  const addLine = (key, selectionType) => {
    ...
    const selling = componentAmount(component, basis, commercial.pricingPolicy ? "costAmountMinor" : "sellingAmountMinor");
    ...
    if (selectionType === "OPTIONAL" && component.replacesComponentKey) {
      ...
      if (replaced && includedKeys.includes(String(component.replacesComponentKey))) {
        costAmountMinor = Math.max(0, costAmountMinor - componentAmount(replaced, basis, "costAmountMinor").amountMinor);
        sellingAmountMinor = Math.max(0, sellingAmountMinor - componentAmount(replaced, basis, "sellingAmountMinor").amountMinor);
      }
    }
    lines.push({
      componentKey: key, type: component.type, name: component.name, selectionType,
      pricingUnit: selling.unit, quantity: selling.quantity,
      costUnitAmountMinor: cost.unitAmountMinor, sellingUnitAmountMinor: selling.unitAmountMinor,
      costAmountMinor, sellingAmountMinor, marginMinor: sellingAmountMinor - costAmountMinor,
  ...
  const agentFeeMinor = policy
    ? (policy.feeType === "FIXED" ? integer(policy.feeAmountMinor ?? 0, "Fixed agent fee") : percentageAmount(costTotalMinor, policy.feePercent, "Agent fee"))
    : Math.max(0, componentSubtotalMinor - costTotalMinor);
  const agentGstMinor = policy ? percentageAmount(agentFeeMinor, policy.gstPercent, "Agent GST") : 0;
  const sellingTotalMinor = policy ? costTotalMinor + agentFeeMinor + agentGstMinor : componentSubtotalMinor;
  return {
    version: "COMPONENTS_V1", currency: commercial.currency || "INR", moneyUnit: "PAISE",
    package: { packageKey: selectedPackage.packageKey, tier: selectedPackage.tier, name: selectedPackage.name },
    basis, selections: { optionalComponentKeys: requestedOptionalKeys }, lines,
    costTotalMinor, componentSubtotalMinor, agentFeeMinor, agentGstMinor,
    sellingTotalMinor, agentAmountMinor: sellingTotalMinor,
    componentMarginMinor: agentFeeMinor,
    requiresRepricing: lines.some((line) => ["ESTIMATED", "REPRICE_REQUIRED"].includes(line.status)),
  };
}

export function deriveTourCommercialSummary(tour) {
  const packages = (tour?.commercial?.packages || []).filter((item) => item.enabled !== false);
  if (tour?.commercial?.version !== "COMPONENTS_V1" || packages.length < 2 || packages.length > 3) {
    throw new Error("Component-priced tours require two or three enabled packages");
  }
  const previews = packages.map((item) => calculateTourCommercials({
    tour, packageKey: item.packageKey, selections: tour.commercial.defaultBasis || {},
  }));
  return {
    ...
    packages: previews.map((item) => ({
      packageKey: item.package.packageKey, tier: item.package.tier,
      costTotalMinor: item.costTotalMinor, sellingTotalMinor: item.sellingTotalMinor,
      agentFeeMinor: item.agentFeeMinor, agentGstMinor: item.agentGstMinor,
      marginMinor: item.componentMarginMinor, requiresRepricing: item.requiresRepricing,
    })),
  };
}
...
const incrementalSupplement = ({ tour, pricing, basis }) => {
  ...
  const agentFeeMinor = policy
    ? (policy.feeType === "FIXED" ? 0 : percentageAmount(costAmountMinor, policy.feePercent, "Agent fee"))
    : 0;
  const agentGstMinor = policy ? percentageAmount(agentFeeMinor, policy.gstPercent, "Agent GST") : 0;
  return {
    unit, quantity, costUnitAmountMinor, costAmountMinor, agentFeeMinor, agentGstMinor,
...
const incrementalRoomUpgrade = ({ tour, targetPricing, includedPricing, basis }) => {
  ...
  const included = incrementalSupplement({ tour: { ...tour, commercial: { ...tour.commercial, pricingPolicy: null } }, pricing: includedPricing, basis });
...
export function calculateTourHotelUnitPrice({ tour, pricing } = {}) {
  ...
  return { amountMinor: priced.sellingTotalMinor, currency: pricing.currency || tour?.commercial?.currency || "INR", unit: priced.unit };
}

const hotelKey = (option) => String(option?.optionKey || option?._id || "");
const roomKey = (room) => String(room?.roomKey || room?._id || "");
const stayKeyFor = (option) => String(option?.stayKey || option?.location || hotelKey(option))
  .trim().toLowerCase().replace(/[^a-z0-9]+/g, "-").replace(/^-+|-+$/g, "");
const packageKeysForRoom = (option, room) => (room?.packageKeys?.length ? room.packageKeys : option?.packageKeys || []).map(String);

const normalizeHotelSelections = ({ hotelSelections, hotelOptionKey, roomOptionKey }) => {
  const raw = Array.isArray(hotelSelections) && hotelSelections.length
    ? hotelSelections
    : (hotelOptionKey ? [{ hotelOptionKey, roomOptionKey }] : []);
...
const resolveHotelSelections = (tour, input) => {
  ...
  return normalizeHotelSelections(input).map((selection) => {
    ...
    if (seenStays.has(resolvedStayKey)) throw new Error(`Choose only one hotel and room for stay '${resolvedStayKey}'`);
    seenStays.add(resolvedStayKey);
    const room = selection.roomOptionKey
      ? (option.rooms || []).find((item) => item?.available !== false && roomKey(item) === selection.roomOptionKey)
      : null;
    if (selection.roomOptionKey && !room) throw new Error("The selected room is not available at this hotel");
...
const priceStaySelections = ({ tour, selections, packageKey, travellers, baseBasis }) => {
  ...
  const hotels = selections.map(({ stayKey, option, room }) => {
    const groups = (tour.hotelOptions || []).filter((candidate) => candidate?.active !== false && stayKeyFor(candidate) === stayKey);
    const selectedIncluded = packageKeysForRoom(option, room).includes(packageKey);
    const includedChoice = selectedIncluded ? { option, room } : groups
      .flatMap((candidate) => (candidate.rooms || []).filter((candidateRoom) => candidateRoom?.available !== false).map((candidateRoom) => ({ option: candidate, room: candidateRoom })))
      .find((candidate) => packageKeysForRoom(candidate.option, candidate.room).includes(packageKey));
    const targetPricing = room?.pricing?.amountMinor != null ? room.pricing : option?.pricing;
    const includedPricing = includedChoice?.room?.pricing?.amountMinor != null
      ? includedChoice.room.pricing
      : includedChoice?.option?.pricing?.amountMinor != null ? includedChoice.option.pricing : null;
    const pricePending = Boolean(!selectedIncluded && (!targetPricing || targetPricing.amountMinor == null));
    const maxAdults = Math.max(1, Number(room?.maxAdults || includedChoice?.room?.maxAdults || 2));
    const stayBasis = {
      ...baseBasis,
    ...
    return {
      ...
      includedOptionName: String(includedChoice?.option?.propertyName || includedChoice?.option?.title || ""),
      ...
      supplement: pricePending
        ? { totalMinor: null, perPersonMinor: null, unit: supplement.unit, quantity: null, status: "PENDING_AGENT_QUOTE" }
        : { ...publicAmount(supplement.sellingTotalMinor, travellers), unit: supplement.unit, quantity: supplement.quantity, status: "CALCULATED" },
    };
  });
...
const normalizeHotelRequests = (tour, hotelRequests = []) => {
  ...
  const stays = new Map((tour?.hotelOptions || []).map((option) => [stayKeyFor(option), option]).filter(([key]) => key));
  const seen = new Set();
  return hotelRequests.slice(0, 12).map((request) => {
    const stayKey = String(request?.stayKey || "").trim().slice(0, 100);
    const stay = stays.get(stayKey);
    if (!stay) throw new RangeError("That destination stay is no longer available for hotel requests");
    if (seen.has(stayKey)) throw new RangeError("Only one hotel request can be submitted for each destination stay");
    seen.add(stayKey);
    const propertyClass = String(request?.propertyClass || "").trim().slice(0, 80);
    const roomType = String(request?.roomType || "").trim().slice(0, 120);
    const requirements = String(request?.requirements || "").trim().slice(0, 600);
    if (!propertyClass && !roomType && !requirements) throw new RangeError("Add a hotel category, room type, or requirement");
    ...
    if (budgetPerNightMinor != null && (!Number.isSafeInteger(budgetPerNightMinor) || budgetPerNightMinor < 0 || budgetPerNightMinor > 100000000)) {
      throw new RangeError("Hotel budget must be a valid amount");
    }
    return {
      stayKey,
      location: String(stay.location || "").trim().slice(0, 120),
      nights: Math.max(0, Number(stay.nights || request?.nights || 0)),
      propertyClass,
      roomType,
      budgetPerNightMinor,
      currency: String(tour?.commercial?.currency || "INR").toUpperCase().slice(0, 3),
      requirements,
      status: "REQUESTED",
    };
  });
...
export function calculateTourCustomizationPreview({ tour, packageKey, hotelSelections = [], hotelRequests = [], hotelOptionKey = "", roomOptionKey = "", travellerCount = 1 } = {}) {
  if (!tour) throw new TypeError("Tour is required");
  ...
  const selections = resolveHotelSelections(tour, { hotelSelections, hotelOptionKey, roomOptionKey });
  const basis = {
  ...
  const pricedStays = priceStaySelections({ tour, selections, packageKey: selectedKey, travellers, baseBasis: basis });
  const customizedTotalMinor = pricedStays.pending ? null : selected.sellingTotalMinor + pricedStays.supplementTotalMinor;
  ...
  const alternatives = (tour.commercial?.packages || [])
    ...
    .map((item) => {
      const commercial = calculateTourCommercials({ tour, packageKey: item.packageKey, selections: basis });
      const alternativeStays = priceStaySelections({ tour, selections, packageKey: String(item.packageKey), travellers, baseBasis: basis });
      const totalMinor = alternativeStays.pending ? null : commercial.sellingTotalMinor + alternativeStays.supplementTotalMinor;
      return { commercial, totalMinor, pending: alternativeStays.pending };
    })
    .map(({ commercial, totalMinor, pending }) => {
      const differenceMinor = customizedTotalMinor == null || pending ? null : totalMinor - customizedTotalMinor;
      const absoluteDifferenceMinor = differenceMinor == null ? null : Math.abs(differenceMinor);
      return {
        packageKey: commercial.package.packageKey,
        packageName: commercial.package.name,
        ...(totalMinor == null ? { totalMinor: null, perPersonMinor: null } : publicAmount(totalMinor, travellers)),
        differenceMinor,
        absoluteDifferenceMinor,
        differencePerPersonMinor: absoluteDifferenceMinor == null ? null : Math.round(absoluteDifferenceMinor / travellers),
        ...
        additionalBenefits: commercial.lines.filter((line) => !selectedComponentKeys.has(String(line.componentKey))).map((line) => line.name).filter(Boolean),
      };
    })
    .sort((left, right) => (left.totalMinor ?? Number.MAX_SAFE_INTEGER) - (right.totalMinor ?? Number.MAX_SAFE_INTEGER));
  ...
  const preview = {
    ...
    customized: customizedTotalMinor == null
