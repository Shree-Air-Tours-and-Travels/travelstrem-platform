...
const resolveRemoteImageUrl = async (sourceUrl) => {
    ...
    try {
        ...
        throw Object.assign(
            new Error(`Could not resolve the Unsplash photo page: ${error.message}`),
            { status: 400 },
        );
    }
};
...
export const importRemoteTourImage = async (sourceUrl) => {
    ...
    if (!isConfigured) {
        throw Object.assign(new Error("Cloudinary must be configured to import an image URL."), {
            status: 503,
        });
    }
    ...
    try {
        ...
        throw Object.assign(new Error(`Could not import the remote image: ${error.message}`), {
...
export const localizeTourImageUrls = async (tour = {}) => {
    ...
    const externalUrls = [
        ...new Set(values.filter((url) => !isCloudinaryImageUrl(url, CLOUDINARY_NAME))),
    ];
    if (externalUrls.length === 0) return tour;

    const results = await Promise.allSettled(
        externalUrls.map(async (url) => [url, await importRemoteTourImage(url)]),
    );
    const configurationError = results.find(
        (result) => result.status === "rejected" && result.reason?.status === 503,
    );
...
const documentFileFilter = (req, file, cb) => {
    ...
    cb(
        allowed.includes(file.mimetype)
            ? null
            : new Error(`Unsupported document type: ${file.mimetype}`),
        allowed.includes(file.mimetype),
    );
};
