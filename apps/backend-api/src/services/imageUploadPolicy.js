...
export const normalizeRemoteImageUrl = (value = "") => {
  ...
  if (!["http:", "https:"].includes(parsed.protocol) || parsed.username || parsed.password) {
    throw Object.assign(new Error("Only public HTTP or HTTPS image URLs are supported."), { status: 400 });
  }

  const hostname = parsed.hostname.toLowerCase().replace(/^\[|\]$/g, "");
  const isLocalHostname = hostname === "localhost" || hostname.endsWith(".localhost");
  const isPrivateIpv4 = /^(?:127\.|10\.|0\.|169\.254\.|192\.168\.|172\.(?:1[6-9]|2\d|3[01])\.)/.test(hostname);
  ...
  if (isLocalHostname || isPrivateIpv4 || isPrivateIpv6) {
    throw Object.assign(new Error("The image URL must be publicly accessible."), { status: 400 });
  }
...
export const isCloudinaryImageUrl = (value = "", cloudName = "") => {
  try {
    ...
    return parsed.hostname.toLowerCase() === "res.cloudinary.com" && owner === String(cloudName || "");
  } catch {
    return false;
  }
};
