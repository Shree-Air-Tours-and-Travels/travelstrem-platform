...
function header(doc, title) {
  ...
  doc.moveDown(0.3);
  doc.fontSize(7).fillColor("#666").text(`${COMPANY.email}  |  ${COMPANY.phone}`, { align: "center" });
  doc.fillColor("#000");
  doc.moveDown(0.5);
...
function footer(doc) {
  ...
  doc.fontSize(7).fillColor("#999").text(
    ...
    50, bottom + 6, { align: "center", width: 495 }
  );
  doc.text(`Generated on ${new Date().toLocaleDateString("en-IN", { dateStyle: "long" })}`,
    50, bottom + 18, { align: "center", width: 495 }
  );
  doc.fillColor("#000");
}
...
export function generateQuotePdf(booking, quote, tour, travelers) {
  ...
  if (quote.couponDiscount > 0) items.push(["Coupon Discount", `-${formatINR(quote.couponDiscount)}`]);
  ...
  items.forEach(([label, amount], i) => {
    ...
    doc.font("Helvetica").fillColor("#000").text(amount, 380, y, { align: "right", width: 120 });
    if (i < items.length - 1) {
      doc.moveTo(60, y + 16).lineTo(540, y + 16).strokeColor("#f5f5f5").stroke();
    }
  });
...
export function generateInvoicePdf(booking, payments, tour) {
  ...
  if (rows.length > 0) {
    ...
    doc.text(remaining > 0 ? `PARTIALLY PAID (₹${remaining.toLocaleString("en-IN")} due)` : "PAID IN FULL",
      380, endY + 8, { align: "right", width: 120 });
  }
...
export function generateBookingPassPdf(booking, travelers, tour) {
  ...
  if (tour?.address) {
    ...
    labelValue(doc, "Location:", [a.line1, a.line2, a.city, a.state, a.zip].filter(Boolean).join(", "));
  }
  ...
  doc.moveDown(0.3);
  doc.fontSize(8).fillColor("#999").text("In case of emergency during the tour, please contact the above number immediately.");
