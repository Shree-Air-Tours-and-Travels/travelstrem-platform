...
try {
    const query = {
        $or: [{ commercial: { $exists: false } }, { "commercial.version": { $exists: false } }],
    };
        ...
        JSON.stringify(
            {
                ...
                legacyToursToMark: legacyCount,
                componentPricedTours: componentCount,
            },
            null,
            2,
        ),
    ...
    if (apply && legacyCount) {
        const result = await Tour.updateMany(query, {
            $set: { commercial: { version: "LEGACY" } },
        });
        console.log(
            JSON.stringify(
