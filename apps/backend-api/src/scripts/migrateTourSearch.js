...
const usedSlugs = new Set((await Tour.find({ slug: { $nin: [null, ""] } }).select("slug").lean()).map((tour) => tour.slug));
let updatedTours = 0;
...
for (const tour of tours) {
  ...
  const average = reviewCount
    ? Number((tour.reviews.reduce((sum, review) => sum + (Number(review.rating) || 0), 0) / reviewCount).toFixed(1))
    : 0;
  const searchTags = (tour.tags || []).map((name) => {
    const normalized = String(name || "").trim();
    const tagSlug = slugify(normalized);
    const type = normalized.toLowerCase() === destinationName.toLowerCase()
      ? "DESTINATION"
      : normalized.toLowerCase() === originName.toLowerCase() ? "ORIGIN" : "CUSTOM";
  ...
  tour.set({
    ...
    destinations: tour.destinations?.length ? tour.destinations : [{
      destinationId: slugify(destinationName), name: destinationName, cityId: slugify(destinationName), cityName: destinationName,
      countryId: slugify(countryName), countryName, sortOrder: 0,
    }],
  ...
  if (tour.startDate && tour.endDate) {
    ...
    const result = await TourDeparture.updateOne(
      { tourId: tour._id, legacyDerived: true },
      {
        ...
        $set: {
          origin: { cityId: slugify(originName), cityName: originName, countryId: "", countryName: "" },
          ...
          pricing: { currency: price.currency || "INR", min: price.min, max: price.max, isFinal: Boolean(price.isFinal), source: price.source || "manual" },
        },
      },
      { upsert: true },
    );
...
console.log(`Tour search migration complete: ${updatedTours} tours normalized, ${createdDepartures} legacy departures created.`);
