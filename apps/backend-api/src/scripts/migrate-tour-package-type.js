...
const MONGO_URI =
    process.env.MONGO_URI || process.env.MONGODB_URI || "mongodb://localhost:27017/travelstrem";
    ...
    while (await cursor.hasNext()) {
        ...
        if (tour.startDate && tour.endDate && (!tour.departures || tour.departures.length === 0)) {
            const departure = {
                ...
                seatsAvailable:
                    tour.availability?.seatsAvailable || tour.availability?.totalSeats || null,
                pricing: {
    ...
    if (DRY_RUN) {
        ...
        for (const op of updates.slice(0, 10)) {
            console.log(
                ...
                JSON.stringify(op.updateOne.update.$set),
            );
        }
