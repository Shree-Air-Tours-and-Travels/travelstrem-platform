...
for (const agency of agencies) {
  ...
  for (const user of users) { user.agencyId = agency._id; user.agencyRole = user.agencyRole === "none" ? "partner_agent" : user.agencyRole; user.accountStatus = user.accountStatus || "active"; await user.save(); }
...
const duplicateDeletionRequests = await AgentDeletionRequest.aggregate([
  ...
  { $sort: { createdAt: -1 } },
  { $group: { _id: "$agentId", keep: { $first: "$_id" }, duplicates: { $push: "$_id" }, count: { $sum: 1 } } },
  { $match: { count: { $gt: 1 } } },
]);
for (const group of duplicateDeletionRequests) {
  await AgentDeletionRequest.updateMany({ _id: { $in: group.duplicates.filter((id) => String(id) !== String(group.keep)) } }, { $set: { status: "cancelled", decisionNotes: "Cancelled by tenancy migration because a newer pending request exists.", decisionDate: new Date() } });
}
