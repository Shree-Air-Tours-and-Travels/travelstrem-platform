...
if (keys.some((key) => !/^[a-zA-Z0-9._-]{3,120}$/.test(key))) throw new Error("Export contains an invalid option-set key");
if (new Set(keys).size !== keys.length) throw new Error("Export contains duplicate option-set keys");
