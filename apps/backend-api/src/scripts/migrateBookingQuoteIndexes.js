...
if (legacy && !(legacy.partialFilterExpression?.bookingId?.$type === "objectId"
  && legacy.partialFilterExpression?.version?.$type === "number")) incompatible.push(legacy.name);

const quoteNumber = byName("quoteNumber_1");
if (quoteNumber && !(quoteNumber.partialFilterExpression?.quoteType === "BOOKING_V2"
  && quoteNumber.partialFilterExpression?.quoteNumber?.$type === "string")) incompatible.push(quoteNumber.name);

const expiry = byName("booking_quote_expiry") || byName("expiresAt_1");
if (expiry && !(expiry.expireAfterSeconds === 0
  && expiry.partialFilterExpression?.quoteType === "BOOKING_V2")) incompatible.push(expiry.name);
...
console.log(JSON.stringify(verified.filter((index) => ["bookingId_1_version_1", "quoteNumber_1", "booking_quote_expiry"].includes(index.name)), null, 2));
