...
export const getClients = async (req, res) => {
  try {
    ...
    return sendJson(res, 500, { status: "error", message: "Failed to fetch clients", error: error.message });
  }
};
...
export const getClient = async (req, res) => {
  try {
    ...
    return sendJson(res, 500, { status: "error", message: "Failed to fetch client", error: error.message });
  }
};
...
export const createClient = async (req, res) => {
  try {
    ...
    if (existing) {
      return sendJson(res, 409, { status: "error", message: `Client with slug "${slug}" already exists` });
    }
    ...
    return sendJson(res, 500, { status: "error", message: "Failed to create client", error: error.message });
  }
};

export const updateClient = async (req, res) => {
  try {
    const { name, slug, status, contactEmail, contactPhone, website, branding, globalBrand } = req.body;
    const update = {};
    ...
    const client = await Client.findByIdAndUpdate(req.params.id, { $set: update }, { new: true, runValidators: true }).lean();
    if (!client) return sendJson(res, 404, { status: "error", message: "Client not found" });
    return sendJson(res, 200, {
      status: "success",
      componentData: { data: { client } },
    ...
    return sendJson(res, 500, { status: "error", message: "Failed to update client", error: error.message });
  }
};
...
export const deleteClient = async (req, res) => {
  try {
    ...
    return sendJson(res, 500, { status: "error", message: "Failed to delete client", error: error.message });
  }
};
...
export const uploadClientLogo = async (req, res) => {
  try {
    ...
    if (!product) return sendJson(res, 400, { status: "error", message: "product query param is required (e.g. ?product=trevio)" });
    ...
    const client = await Client.findByIdAndUpdate(id, { $set: { [updatePath]: url } }, { new: true }).lean();
    if (!client) return sendJson(res, 404, { status: "error", message: "Client not found" });
    ...
    return sendJson(res, 500, { status: "error", message: "Logo upload failed", error: error.message });
  }
};
