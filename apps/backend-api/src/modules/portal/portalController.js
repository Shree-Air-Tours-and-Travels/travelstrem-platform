...
const readJsonConfig = (filename) => {
    const configPath = path.resolve(configDir, filename);
...
const resolvePageConfig = (req) => {
    const json = readJsonConfig("pageConfig.json");
...
export const getUserSession = async (req, res) => {
    try {
        const json = readJsonConfig("userSession.json");
        const pathname = req.query.pathname || "/";
...
export const getHeaderConfig = async (req, res) => {
    try {
        const json = readJsonConfig("header.json");
