...
export async function latestQuoteDocument(bookingId, quoteVersion = null) {
  return BookingDocument.findOne({ bookingId, type: DOCUMENT_TYPE.QUOTE, ...(quoteVersion == null ? {} : { quoteVersion: Number(quoteVersion) }) }).sort({ quoteVersion: -1, uploadedAt: -1 });
}
...
export default { latestQuoteDocument, readQuoteDocument, getQuoteDocumentSignedUrl, resolveQuoteDocumentPath, quoteUploadDirectory };
