...
const bookingQuoteSchema = new Schema(
    {
        quoteType: {
            type: String,
            enum: ["LEGACY", "BOOKING_V2", "FINANCIAL"],
            default: "LEGACY",
            index: true,
        },
        ...
        activitiesPrice: { type: Number, default: 0 },
        mealsPrice: { type: Number, default: 0 },
        amountPayableNow: { type: Number, default: 0 },
        ...
        status: {
            type: String,
            enum: [...new Set([...QUOTE_STATUS_LIST, ...V2_STATUSES])],
            default: "READY",
            index: true,
        },
    },
...
bookingQuoteSchema.pre("validate", function validateQuoteType(next) {
    if (this.quoteType === "BOOKING_V2") {
        if (
            (!this.userId && !this.guestSessionId) ||
            !this.tourId ||
            !this.pricing ||
            !this.expiresAt
        )
            return next(new Error("V2 booking quote is incomplete"));
        if (!V2_STATUSES.includes(this.status)) this.status = "ACTIVE";
    } else if (this.quoteType === "LEGACY" && (!this.bookingId || this.version == null))
        return next(new Error("Legacy booking quote is incomplete"));
    else if (
        this.quoteType === "FINANCIAL" &&
        (!this.contextType || !this.contextId || !this.financialSnapshot || !this.configSnapshot)
    )
        return next(new Error("Financial quote is incomplete"));
    return next();
});

bookingQuoteSchema.virtual("id").get(function id() {
...
bookingQuoteSchema.index(
    { expiresAt: 1 },
    {
        name: "booking_quote_expiry",
        expireAfterSeconds: 0,
        partialFilterExpression: { quoteType: "BOOKING_V2" },
    },
);
bookingQuoteSchema.index(
    { idempotencyKey: 1 },
