...
export const resolveDepartureOption = (options = [], selectedValue = "") => {
  ...
  const exact = options.find((option) => [option?.value, option?.label]
    .some((candidate) => normalizeText(candidate) === selectedText));
  if (exact) return exact;

  const selectedDateRange = dateRangeKey(selectedValue);
  if (!selectedDateRange) return null;
  return options.find((option) => [option?.value, option?.label]
    .some((candidate) => dateRangeKey(candidate) === selectedDateRange)) || null;
};
...
export const normalizeMongoId = (value) => {
  ...
  const ordered = Array.isArray(bytes)
    ? bytes
    : Object.keys(bytes).sort((a, b) => Number(a) - Number(b)).map((key) => bytes[key]);
