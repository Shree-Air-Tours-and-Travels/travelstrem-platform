...
const AGENCY_POPULATE = { path: "agencyId", select: "agencyName partnerAgencyRef logo website address status" };
...
const TourRepository = {
  ...
  findByIdAndUpdate(id, payload, options) {
    return Tour.findByIdAndUpdate(id, payload, options).populate(OWNER_POPULATE).populate(AGENCY_POPULATE);
  },
