...
const slugifySearchValue = (value = "") =>
    String(value)
        .trim()
        .toLowerCase()
        .replace(/&/g, " and ")
...
const extraSchema = new Schema(
    {
        ...
        category: {
            type: String,
            enum: ["activity", "transfer", "meal", "visa", "insurance", "other", ""],
            default: "",
        }, // add-on type
...
const tourDepartureSchema = new Schema(
    {
        ...
        status: {
            type: String,
            enum: DEPARTURE_STATUS_LIST,
            default: DEPARTURE_STATUS.ACTIVE,
            index: true,
        },
...
const commercialComponentSchema = new Schema(
    {
        ...
        status: {
            type: String,
            enum: ["ESTIMATED", "CONFIRMED", "REPRICE_REQUIRED"],
            default: "CONFIRMED",
        },
        pricing: {
...
const commercialSchema = new Schema(
    {
        ...
        defaultBasis: {
            ...
            vehicles: { type: Number, min: 1, default: 1 },
            nights: { type: Number, min: 0, default: 1 },
        ...
        derived: {
            ...
            displayMode: {
                type: String,
                enum: ["FINAL", "STARTING_FROM", "ESTIMATED"],
                default: "ESTIMATED",
            },
...
const tourSchema = new Schema(
    {
        ...
        packageType: {
            ...
            default: PACKAGE_TYPE.FIXED_DEPARTURE,
            index: true,
        },
...
tourSchema.methods.getCurrentPrice = function (date = new Date()) {
    ...
    if (
        this.packageType === PACKAGE_TYPE.FIXED_DEPARTURE &&
...
tourSchema.pre("validate", function normalizeSearchMetadata() {
    ...
    if (this.group?.max == null && this.maxGroupSize != null)
        this.set("group.max", this.maxGroupSize);
    ...
    if (this.commercial?.version === "COMPONENTS_V1") {
        const packageKeys = new Set(
            (this.commercial.packages || [])
                .map((item) => String(item.packageKey || ""))
                .filter(Boolean),
        );
        const assignedStayPackages = new Set();
        const stayGroups = new Map();
        (this.hotelOptions || []).forEach((option, index) => {
            const normalizedStayKey = String(
                option.stayKey || option.location || option.optionKey || "",
            )
                .trim()
                .toLowerCase()
                .replace(/[^a-z0-9]+/g, "-")
                .replace(/^-+|-+$/g, "");
            const normalizedLocation = String(option.location || "")
                .trim()
                .toLowerCase()
                .replace(/[^a-z0-9]+/g, "-")
                .replace(/^-+|-+$/g, "");
            const stayGroup = stayGroups.get(normalizedStayKey);
            if (stayGroup && stayGroup.location !== normalizedLocation)
                this.invalidate(
                    `hotelOptions.${index}.stayKey`,
                    `Hotels grouped under '${normalizedStayKey}' must be in the same location`,
                );
            if (stayGroup && stayGroup.nights !== Number(option.nights || 0))
                this.invalidate(
                    `hotelOptions.${index}.nights`,
                    `Hotels grouped under '${normalizedStayKey}' must cover the same number of nights`,
                );
            if (!stayGroup && normalizedStayKey)
                stayGroups.set(normalizedStayKey, {
                    location: normalizedLocation,
                    nights: Number(option.nights || 0),
                });
            const invalidKey = (option.packageKeys || [])
                .map(String)
                .find((key) => !packageKeys.has(key));
            if (invalidKey)
                this.invalidate(
                    `hotelOptions.${index}.packageKeys`,
                    `Hotel option references unknown package '${invalidKey}'`,
                );
            const assignedPackages = new Set();
            (option.rooms || []).forEach((room, roomIndex) => {
                const invalidRoomKey = (room.packageKeys || [])
                    .map(String)
                    .find((key) => !packageKeys.has(key));
                if (invalidRoomKey)
                    this.invalidate(
                        `hotelOptions.${index}.rooms.${roomIndex}.packageKeys`,
                        `Room option references unknown package '${invalidRoomKey}'`,
                    );
                (room.packageKeys || []).map(String).forEach((key) => {
                    if (assignedPackages.has(key))
                        this.invalidate(
                            `hotelOptions.${index}.rooms.${roomIndex}.packageKeys`,
                            `Only one room per hotel can be included in package '${key}'`,
                        );
                    assignedPackages.add(key);
                    const stayKey = normalizedStayKey;
                    const stayPackageKey = `${stayKey}:${key}`;
                    if (stayKey && assignedStayPackages.has(stayPackageKey))
                        this.invalidate(
                            `hotelOptions.${index}.rooms.${roomIndex}.packageKeys`,
                            `Only one hotel room per stay can be included in package '${key}'`,
                        );
