const VISIBLE_KEYS = new Set([
    "title", "subtitle", "description", "label", "help", "placeholder",
    "message", "text", "note", "addLabel", "emptyLabel", "nextActionLabel",
    "confirmLabel", "cancelLabel", "ariaLabel",
]);
...
export const createBuilderLabelContract = (structure) => {
    ...
    const visit = (value, path = []) => {
        ...
        return Object.fromEntries(Object.entries(value).map(([key, child]) => {
            if (typeof child === "string" && !key.endsWith("Ref") && (VISIBLE_KEYS.has(key) || /Label$/.test(key))) {
