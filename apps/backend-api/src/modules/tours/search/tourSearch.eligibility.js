    ...
    const season = (tour.seasonalPricing || [])
        .filter(
            (item) =>
                item.startDate &&
                item.endDate &&
                new Date(item.startDate) <= target &&
                new Date(item.endDate) >= target,
        )
...
export const getEligibleDeparturePrices = (tour, departures = [], filters = {}) => {
    const inventory = departures.length
        ? departures
        : [
              {
                  status: "legacy",
    ...
    if (
        filters.travellers != null &&
        Number(tour.group?.max ?? tour.maxGroupSize) < filters.travellers
    )
        return [];
    return inventory
        .filter((departure) => {
            ...
            if (
                ...
                departure.availableSeats < filters.travellers
            )
                return false;
