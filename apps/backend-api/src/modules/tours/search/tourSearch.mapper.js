...
const displayText = (value, fallback = "") => {
  ...
  if (typeof value === "object") {
    ...
    return [value.city, value.country].map((item) => displayText(item)).filter(Boolean).join(", ") || fallback;
  }
  return fallback;
};

const mapFacetOptions = (items = []) => items
...
export const mapTourSearchCard = (item = {}) => {
  ...
  return {
    ...
    route: {
      ...
      destination: route.destination ? { ...route.destination, name: displayText(route.destination.name) } : null,
    },
    ...
    availability: item.availability || { availableSeats: null, departureCount: 0, nextDepartureDate: null },
    ...
    agency: item.agency ? { ...item.agency, name: displayText(item.agency.name) } : { id: "", name: "", logo: "" },
...
export const mapTourSearchResult = (aggregation = {}, search) => {
  ...
  return {
    ...
    facets: {
      price: aggregation.price?.[0] ? { min: aggregation.price[0].min ?? 0, max: aggregation.price[0].max ?? 0 } : { min: 0, max: 0 },
      duration: aggregation.duration?.[0] ? { minDays: aggregation.duration[0].minDays ?? 0, maxDays: aggregation.duration[0].maxDays ?? 0 } : { minDays: 0, maxDays: 0 },
