...
const slugCandidates = (values = []) => [...new Set(values.flatMap((value) => {
...
const sortStage = (sort) => {
  ...
  const sorts = {
    [TOUR_SEARCH_SORT.RECOMMENDED]: { featured: -1, trending: -1, "metrics.popularityScore": -1, createdAt: -1, ...stableId },
    ...
    [TOUR_SEARCH_SORT.POPULAR]: { "metrics.popularityScore": -1, "metrics.bookings": -1, createdAt: -1, ...stableId },
    [TOUR_SEARCH_SORT.TRENDING]: { trending: -1, "metrics.trendScore": -1, createdAt: -1, ...stableId },
    [TOUR_SEARCH_SORT.RATING]: { _ratingAverage: -1, _ratingCount: -1, createdAt: -1, ...stableId },
  };
...
const buildDepartureEligibility = (search) => {
  ...
  if (originCandidates.length) {
    conditions.push({
      $or: [
        { $in: [{ $toLower: { $ifNull: ["$$departure.origin.cityId", ""] } }, originCandidates] },
        { $in: [{ $toLower: { $ifNull: ["$$departure.origin.cityName", ""] } }, originCandidates] },
      ],
    });
  }
  return conditions;
...
const facetProjection = {
  ...
  route: {
    origin: {
      id: { $ifNull: [{ $arrayElemAt: ["$_eligibleDepartures.origin.cityId", 0] }, "$_originKey"] },
      name: { $ifNull: [{ $arrayElemAt: ["$_eligibleDepartures.origin.cityName", 0] }, "$_originName"] },
    },
...
export const buildTourSearchPipeline = (search, { departureCollection = "tourdepartures", agencyCollection = "partneragencies" } = {}) => {
  ...
  const pipeline = [
    ...
    },
    {
      $set: {
        ...
        _ratingAverage: {
          $ifNull: [
            "$rating.average",
            { $cond: [{ $gt: [{ $size: { $ifNull: ["$reviews", []] } }, 0] }, { $avg: "$reviews.rating" }, 0] },
          ],
        },
        ...
        _matchingSeasons: {
          $filter: {
            ...
            as: "season",
            cond: { $and: [{ $lte: ["$$season.startDate", priceDate] }, { $gte: ["$$season.endDate", priceDate] }] },
          },
        },
      },
    },
      ...
      $set: {
        _candidateDepartures: {
          $cond: [
            ...
            [{
              status: "legacy",
              origin: { cityId: "$_originKey", cityName: "$_originName", countryId: "", countryName: "" },
    ...
    { $match: { "_eligibleDepartures.0": { $exists: true } } },
    {
      $set: {
        ...
        _currency: { $ifNull: [{ $arrayElemAt: ["$_eligibleDepartures.pricing.currency", 0] }, "$price.currency"] },
        _priceIsFinal: { $allElementsTrue: ["$_eligibleDepartures.pricing.isFinal"] },
        _originFacetValues: {
          $setUnion: [{ $map: { input: "$_eligibleDepartures", as: "departure", in: { id: { $toLower: { $ifNull: ["$$departure.origin.cityId", "$$departure.origin.cityName"] } }, name: "$$departure.origin.cityName" } } }, []],
        },
      },
    },
  ];
  ...
  if (filters.duration.minDays != null || filters.duration.maxDays != null) {
    ...
    if (filters.duration.minDays != null) dynamicMatch._durationDays.$gte = filters.duration.minDays;
    if (filters.duration.maxDays != null) dynamicMatch._durationDays.$lte = filters.duration.maxDays;
  }
  ...
  if (tagCandidateGroups.length) {
    pipeline.push({ $match: { $and: tagCandidateGroups.map((candidates) => ({ _tagSlugs: { $in: candidates } })) } });
  }
  ...
  pipeline.push({
    $facet: {
      ...
      total: [{ $count: "count" }],
      price: [{ $group: { _id: null, min: { $min: "$_priceMin" }, max: { $max: "$_priceMax" } } }],
      duration: [{ $group: { _id: null, minDays: { $min: "$_durationDays" }, maxDays: { $max: "$_durationDays" } } }],
      origins: [{ $unwind: "$_originFacetValues" }, { $group: { _id: "$_originFacetValues.id", name: { $first: "$_originFacetValues.name" }, count: { $sum: 1 } } }, { $sort: { name: 1 } }],
      destinations: [{ $group: { _id: "$_destinationKey", name: { $first: "$_destinationName" }, count: { $sum: 1 } } }, { $sort: { name: 1 } }],
      countries: [{ $group: { _id: "$_countryKey", name: { $first: "$_countryName" }, count: { $sum: 1 } } }, { $sort: { name: 1 } }],
      agencies: [{ $match: { _agencyKey: { $nin: [null, ""] } } }, { $group: { _id: "$_agencyKey", name: { $first: "$_agencyName" }, logo: { $first: "$_agencyLogo" }, count: { $sum: 1 } } }, { $sort: { name: 1 } }],
      tags: [{ $unwind: "$_tagDtos" }, { $group: { _id: { $ifNull: ["$_tagDtos.slug", "$_tagDtos.id"] }, name: { $first: "$_tagDtos.name" }, type: { $first: "$_tagDtos.type" }, count: { $sum: 1 } } }, { $sort: { count: -1, name: 1 } }],
    },
  });
