...
import { normalizeTrip } from "../services/tripService.js";
...
const findTripByRef = async (tripRef) => {
    ...
    if (/^[0-9a-fA-F]{24}$/.test(ref)) {
        const byId = await TripRepository.findOne({ _id: ref });
        if (byId) return byId;
    }
    const bySlug = await TripRepository.findBySlug(ref);
    if (bySlug) return bySlug;
    const byTitle = await TripRepository.findOne({
