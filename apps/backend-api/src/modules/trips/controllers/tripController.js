...
const loadTrips = async (req) => {
    try {
        ...
        console.error("[TripController] Failed to load trips:", error.message);
        return {
            trips: [],
            pagination: {
                page: 1,
...
export const getTripHome = async (req, res) => {
    ...
    return res.status(200).json({
        ...
        message: "Trip home fetched successfully",
    });
};
...
export const getTrip = async (req, res) => {
    try {
        const trip = await tripService.findBySlug(req.params.tripRef);
        if (!trip) return res.status(404).json({ status: "error", message: "Trip not found" });
        return res.status(200).json({
            status: "success",
            component: { data: { trip: tripService.normalize(trip) } },
        });
    } catch (error) {
...
export const calculateTripPricing = async (trip, body = {}) => {
    ...
    const tripData = tripService.normalize(trip);
...
export const getTripPricing = async (req, res) => {
    try {
        const trip = await tripService.findBySlug(req.params.tripRef);
        if (!trip) return res.status(404).json({ status: "error", message: "Trip not found" });
        const { framework, ...publicResult } = await calculateTripPricing(trip, req.body || {});
        return res.status(200).json({
            status: "success",
            component: { data: publicResult },
        });
    } catch (error) {
        console.error("getTripPricing error:", error);
        return res
            .status(500)
...
export const getTripAvailability = async (req, res) => {
    try {
        const trip = await tripService.findBySlug(req.params.tripRef);
        if (!trip) return res.status(404).json({ status: "error", message: "Trip not found" });
        const normalized = tripService.normalize(trip);
        return res.status(200).json({
            status: "success",
