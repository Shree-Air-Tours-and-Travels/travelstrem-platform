...
const normalizeTripForWidget = (trip = {}) => {
  ...
  return {
    ...
    itinerary: (trip.itinerary || []).map((item) => ({
      ...
      summary: item.summary || (Array.isArray(item.activities) ? item.activities.join(", ") : ""),
  ...
  try {
    ...
    if (pageKey === "trevio-remote/details") {
      ...
      if (tripRef) {
        ...
        if (tripDoc) {
          const tripObj = tripDoc.toObject ? tripDoc.toObject({ virtuals: true }) : tripDoc;
          ...
          switch (fileName) {
            ...
            case "tour-highlights.json":
              widget.component.data.highlights = (normalized.chips || []).map((chip, i) => ({
                title: chip,
                short: "",
                icon: "",
                order: i,
            ...
            case "itinerary-timeline.json":
              widget.component.data.itinerary = [...(normalized.itinerary || [])].sort((a, b) => Number(a.day) - Number(b.day));
              break;
            ...
            case "similar-tours.json": {
              ...
              const currentTags = new Set((normalized.tags || []).map((t) => String(t).toLowerCase()));
              widget.component.data.tours = (Array.isArray(allTrips) ? allTrips : [])
                ...
                .sort((a, b) => {
                  const aScore = (a.tags || []).filter((t) => currentTags.has(String(t).toLowerCase())).length;
                  const bScore = (b.tags || []).filter((t) => currentTags.has(String(t).toLowerCase())).length;
