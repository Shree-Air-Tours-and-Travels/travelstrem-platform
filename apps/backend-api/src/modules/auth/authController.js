...
import mailer from "../../utils/mailer.js"; // optional helper for sending emails
import fs from "fs";
import path from "path";
import config from "../../config/index.js"; // single source of truth
...
export const register = async (req, res) => {
    try {
        ...
        if (requestedRole === "admin") {
            ...
            // In production, ADMIN_CREATION_SECRET should be set; comparison is safe here.
            if (!adminSecretReceived || adminSecretReceived !== ADMIN_CREATION_SECRET) {
                if (DEBUG) {
                    console.warn("[register] admin creation secret mismatch or missing (provided:", !!adminSecretReceived, "envPresent:", !!ADMIN_CREATION_SECRET, ")");
                }
...
export const forgotPassword = async (req, res) => {
    try {
        ...
        if (typeof User === "undefined") {
            console.error("[forgotPassword] User model is not defined");
            return res.status(500).json({ message: "Server misconfiguration: User model missing." });
        }
...
export const resetPassword = async (req, res) => {
    try {
        ...
        if (typeof User === "undefined") {
            console.error("[resetPassword] User model is not defined");
            return res.status(500).json({ message: "Server misconfiguration: User model missing." });
        }
