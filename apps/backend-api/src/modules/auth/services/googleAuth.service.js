...
const assertConfigured = () => {
  if (!config.GOOGLE_AUTH_ENABLED || !config.GOOGLE_CLIENT_ID || !config.GOOGLE_CLIENT_SECRET) {
    throw new AuthServiceError("GOOGLE_AUTH_NOT_CONFIGURED", "Google authentication is currently unavailable.", 503);
  }
};

export const beginGoogleAuthentication = async ({ portal: rawPortal, returnTo, ipAddress = "", userAgent = "" }) => {
  assertConfigured();
...
export const completeGoogleAuthentication = async ({ state, code }) => {
  assertConfigured();
  if (!state || !code) throw new AuthServiceError("GOOGLE_AUTH_FAILED", "Unable to authenticate with Google.", 400);
  ...
  if (!transaction) throw new AuthServiceError("OAUTH_STATE_INVALID", "The Google login request is invalid or expired.", 400);
  ...
  if (!tokens.id_token) throw new AuthServiceError("GOOGLE_AUTH_FAILED", "Google did not return a valid identity token.", 401);

  const ticket = await client.verifyIdToken({ idToken: tokens.id_token, audience: config.GOOGLE_CLIENT_ID });
  const claims = ticket.getPayload();
  if (!claims || claims.nonce !== transaction.nonce || !["accounts.google.com", "https://accounts.google.com"].includes(claims.iss)) {
    throw new AuthServiceError("GOOGLE_ID_TOKEN_INVALID", "Google identity validation failed.", 401);
  }
