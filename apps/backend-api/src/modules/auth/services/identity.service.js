...
const ensurePortalAccess = (user, portal) => {
    if (!portalAllowsRole(portal, user.role)) {
        throw new AuthServiceError(
            ...
            `This account does not have access to the ${portal} portal.`,
            403,
        );
    }
    ...
    if (!user) {
        ...
        if (!user && portal !== "customer") {
            throw new AuthServiceError(
                ...
                "An existing portal account is required before Google can be linked.",
                403,
            );
        }
        if (!user) {
...
export const authenticateWithMobile = async ({ phoneNumber, portal = "customer" }) => {
    ...
    if (!user) {
        ...
        if (!user && portal !== "customer") {
            throw new AuthServiceError(
                "PORTAL_ACCOUNT_REQUIRED",
                "An existing verified portal account is required before mobile can be linked.",
                403,
            );
        }
        if (!user) {
        ...
        if (String(identity.userId) !== String(user._id)) {
            throw new AuthServiceError(
                "IDENTITY_ALREADY_LINKED",
                "This mobile number is already linked to another user.",
                409,
            );
        }
    }
    ...
    await AuthIdentity.updateOne(
        { _id: identity._id },
        { $set: { verified: true, lastAuthenticatedAt: new Date() } },
    );
    return user;
};
...
export const linkIdentity = async ({
    ...
    if (!verified)
        throw new AuthServiceError(
            ...
            "Identity ownership must be verified before linking.",
            400,
        );
    const identity = await createIdentitySafely({
        userId,
        provider,
        providerUserId,
        providerEmail,
