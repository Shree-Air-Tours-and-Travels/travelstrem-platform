...
export const normalizeMobileNumber = (input, defaultCountry = "IN") => {
    ...
    if (!parsed?.isValid())
        throw new AuthServiceError("INVALID_MOBILE_NUMBER", "Enter a valid mobile number.", 400);
...
export const requestMobileOtp = async ({
    ...
    const recent = await MobileOtpChallenge.findOne({
        phoneNumber,
    ...
    const recentCount = await MobileOtpChallenge.countDocuments({
        phoneNumber,
    ...
    const otp =
        config.IS_DEVELOPMENT && config.MOBILE_AUTH_DEV_MODE
