...
export const listMasterOptionSets = async (req, res) => {
  try {
    ...
    return res.status(500).json({ status: "error", message: "Master option sets could not be loaded" });
  }
};
...
export const upsertMasterOptionSet = async (req, res) => {
  ...
  try {
    ...
    return res.status(200).json({ status: "success", data: { optionSet }, message: "Master options saved" });
  } catch (error) {
    return res.status(400).json({ status: "error", message: error.message || "Master options could not be saved" });
  }
};
