...
class MasterDataService {
    async getOptionSet(key) {
        ...
        try {
            ...
            console.error(
                `[MasterDataService] Failed to resolve option set "${key}":`,
                error.message,
            );
            return [];
        }
    ...
    async getOptionSets(keys = []) {
        const entries = await Promise.all(
            keys.map(async (key) => [key, await this.getOptionSet(key)]),
        );
    ...
    async hydrateDataScope(component = {}) {
        ...
        const options = {
            ...
            ...Object.fromEntries(
                entries.map(([localKey, setKey]) => [localKey, sets[setKey] || []]),
            ),
        };
        return {
            ...component,
    ...
    async ensureDefaults() {
        const results = await Promise.all(
            Object.entries(DEFAULT_OPTION_SETS).map(([key, definition]) =>
