...
describe("tenant RBAC policy", () => {
  ...
  test("permission denials override grants and role permissions", () => {
    const permissions = permissionsFor({ role: "agent", agencyRole: "partner_admin", permissionDenials: [PERMISSIONS.TRIP_PUBLISH] });
  ...
  test("tenant query cannot be overridden by caller data", () => {
    ...
    expect(tenantQuery(req, { status: "active", agencyId: "agency-b" })).toEqual({ status: "active", agencyId: "agency-a" });
  });
  ...
  test("master tenant queries deliberately remain platform-scoped", () => {
    expect(tenantQuery({ access: { isMaster: true } }, { status: "active" })).toEqual({ status: "active" });
  });
});
