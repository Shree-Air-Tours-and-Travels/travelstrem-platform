...
const generateCodeVerifierAsync = jest.fn(async () => ({ codeVerifier: "verifier", codeChallenge: "challenge" }));
class OAuth2Client {
...
jest.unstable_mockModule("google-auth-library", () => ({ OAuth2Client, CodeChallengeMethod: { S256: "S256" } }));
jest.unstable_mockModule("../../config/index.js", () => ({ default: {
...
jest.unstable_mockModule("../../modules/auth/models/OAuthTransaction.js", () => ({ default: transactionModel }));
jest.unstable_mockModule("../../modules/auth/services/identity.service.js", () => ({
  AuthServiceError: class AuthServiceError extends Error { constructor(code, message, status) { super(message); this.code = code; this.status = status; } },
  authenticateWithGoogle,
}));

const { beginGoogleAuthentication, completeGoogleAuthentication } = await import("../../modules/auth/services/googleAuth.service.js");
...
test("post-login redirects cannot point back to AuthTREM or across portals", async () => {
  transactionModel.create.mockResolvedValue({});
  await beginGoogleAuthentication({ portal: "customer", returnTo: "http://localhost:3003/login" });
  expect(transactionModel.create).toHaveBeenLastCalledWith(expect.objectContaining({ returnTo: "http://localhost:3006" }));

  await beginGoogleAuthentication({ portal: "admin", returnTo: "http://localhost:3006/" });
  expect(transactionModel.create).toHaveBeenLastCalledWith(expect.objectContaining({ returnTo: "http://localhost:3002" }));
});

test("OAuth callback rejects a state mismatch", async () => {
  transactionModel.findOneAndUpdate.mockResolvedValue(null);
  await expect(completeGoogleAuthentication({ state: "wrong", code: "code" }))
...
test("valid callback verifies nonce and authenticates the internal user", async () => {
  ...
  await expect(completeGoogleAuthentication({ state: "state", code: "code" }))
  ...
  expect(authenticateWithGoogle).toHaveBeenCalledWith(expect.objectContaining({ portal: "customer" }));
});

test("callback rejects an invalid nonce", async () => {
  transactionModel.findOneAndUpdate.mockResolvedValue({ codeVerifier: "verifier", nonce: "expected", portal: "customer", returnTo: "http://localhost:3006/" });
  getToken.mockResolvedValue({ tokens: { id_token: "signed-id-token" } });
  verifyIdToken.mockResolvedValue({ getPayload: () => ({ nonce: "different", iss: "https://accounts.google.com" }) });
  await expect(completeGoogleAuthentication({ state: "state", code: "code" }))
