...
jest.unstable_mockModule("../../modules/auth/models/AuthIdentity.js", () => ({ default: identityModel }));
jest.unstable_mockModule("../../modules/auth/models/User.js", () => ({ default: userModel }));

const { authenticateWithGoogle, authenticateWithMobile, linkIdentity } = await import("../../modules/auth/services/identity.service.js");
...
test("existing Google identity logs in its linked user", async () => {
  ...
  await expect(authenticateWithGoogle({ claims: googleClaims, portal: "customer" })).resolves.toBe(user);
...
test("new verified Google identity creates a user and provider link", async () => {
  ...
  expect(userModel.create).toHaveBeenCalledWith(expect.objectContaining({ email: googleClaims.email, emailVerified: true, role: "member" }));
  expect(identityModel.create).toHaveBeenCalledWith(expect.objectContaining({ provider: "GOOGLE", providerUserId: googleClaims.sub, userId: user._id }));
});
...
test("an admin Google identity still cannot enter PartnerTREM", async () => {
  ...
  await expect(authenticateWithGoogle({ claims: googleClaims, portal: "partner" }))
    .rejects.toMatchObject({ code: "PORTAL_ACCESS_DENIED" });
});

test("unverified identities cannot be linked", async () => {
  await expect(linkIdentity({ userId: "user-1", provider: "MOBILE", providerUserId: "+919876543210", verified: false }))
...
test("verified mobile OTP logs in an existing mobile identity", async () => {
  ...
  userModel.findById.mockResolvedValue(user);
  await expect(authenticateWithMobile({ phoneNumber: "+919876543210", portal: "customer" })).resolves.toBe(user);
  expect(userModel.create).not.toHaveBeenCalled();
});
...
test("verified mobile OTP creates a new user and MOBILE identity", async () => {
  ...
  expect(userModel.create).toHaveBeenCalledWith(expect.objectContaining({ mobile: "+919876543210", mobileVerified: true }));
  expect(identityModel.create).toHaveBeenCalledWith(expect.objectContaining({ provider: "MOBILE", providerUserId: "+919876543210" }));
});
