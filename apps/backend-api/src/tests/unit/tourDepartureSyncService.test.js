...
describe("derived tour departure synchronization", () => {
  ...
  it("maps a published tour to active searchable departure inventory", () => {
    expect(buildDerivedTourDeparture(tour)).toEqual(expect.objectContaining({
      tourId: "tour-1",
      status: "active",
      capacity: 20,
      availableSeats: 14,
      origin: { cityId: "new-delhi", cityName: "New Delhi", countryId: "india", countryName: "India" },
      pricing: { min: 8999, max: 13999, currency: "INR", isFinal: false, source: "manual" },
      legacyDerived: true,
    }));
  });

  it("marks zero-seat and cancelled tours unavailable", () => {
    expect(buildDerivedTourDeparture({ ...tour, availability: { totalSeats: 20, seatsAvailable: 0 } }).status).toBe("sold_out");
    expect(buildDerivedTourDeparture({ ...tour, status: "cancelled" }).status).toBe("cancelled");
  });
