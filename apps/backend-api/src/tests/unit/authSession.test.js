...
const refreshModel = {
  ...
  create: jest.fn(async (payload) => {
    const record = { ...payload, revokedAt: null, createdAt: new Date(), save: jest.fn().mockResolvedValue(undefined) };
  ...
  updateMany: jest.fn(async (query, update) => {
    sessions.filter((candidate) => matches(candidate, query)).forEach((record) => Object.assign(record, update.$set || {}));
    return { acknowledged: true };
  }),
};
const user = {
...
jest.unstable_mockModule("../../config/index.js", () => ({ default: {
  JWT: { accessSecret: "test-access-secret-at-least-32-characters", accessExpires: "15m", refreshExpires: "30d" },
} }));
jest.unstable_mockModule("../../modules/auth/models/RefreshToken.js", () => ({ default: refreshModel }));
jest.unstable_mockModule("../../modules/auth/models/User.js", () => ({ default: userModel }));

const { createSession, rotateSession, revokeCurrentSession } = await import("../../modules/auth/services/session.service.js");
...
const makeResponse = () => {
  ...
  return {
    cookies,
    cookie: jest.fn((name, value) => { cookies[name] = value; }),
  };
};
...
test("refresh token rotates and the previous session is revoked", async () => {
  ...
  const result = await rotateSession({ req: makeRequest({ [names.refresh]: oldRefresh }), res: nextRes, portal: "customer" });
  expect(result.authenticated).toBe(true);
...
test("invalid refresh token fails", async () => {
  const names = getPortalCookieNames("customer");
  await expect(rotateSession({ req: makeRequest({ [names.refresh]: "invalid" }), res: makeResponse(), portal: "customer" }))
    .resolves.toBeNull();
});
...
test("replayed revoked refresh token revokes its token family", async () => {
  ...
  const names = getPortalCookieNames("customer");
  const oldRefresh = firstRes.cookies[names.refresh];
  await rotateSession({ req: makeRequest({ [names.refresh]: oldRefresh }), res: makeResponse(), portal: "customer" });
  await expect(rotateSession({ req: makeRequest({ [names.refresh]: oldRefresh }), res: makeResponse(), portal: "customer" })).resolves.toBeNull();
...
test("logout revokes the presented session and clears both cookies", async () => {
  ...
  await revokeCurrentSession(makeRequest({ [names.refresh]: firstRes.cookies[names.refresh] }), logoutRes, "customer");
  expect(sessions[0].revokedAt).toBeInstanceOf(Date);
