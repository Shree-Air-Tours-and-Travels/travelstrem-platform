...
describe("FinancialEngine core", () => {
    ...
    test("calculates partial refunds and settlement adjustments from snapshots", async () => {
        const financials = await FinancialEngine.calculateBookingFinancials({
            agentAmountMinor: 10_000_000,
            config: config(),
        });
        const refund = await FinancialEngine.calculateRefund({
            financials,
            amountMinor: 5_590_000,
            config: config(),
        });
        const settlement = await FinancialEngine.calculateSettlement({
            financials,
            paidMinor: 11_180_000,
            refundedMinor: 5_590_000,
        });
        ...
        const rows = [
            {
                scopeType: "GLOBAL",
                scopeId: "default",
                config: { commission: { rateBasisPoints: 500 }, gatewayFee: { enabled: true } },
            },
            {
                scopeType: "AGENCY",
                scopeId: "agency-1",
                config: { commission: { rateBasisPoints: 700 } },
            },
            {
                scopeType: "TOUR",
                scopeId: "tour-1",
                config: { commission: { rateBasisPoints: 900 } },
            },
            {
                scopeType: "BOOKING",
                scopeId: "booking-1",
                config: { commission: { rateBasisPoints: 1100 } },
            },
        ];
        const resolved = await resolveFinancialConfig(
            { agencyId: "agency-1", tourId: "tour-1", bookingId: "booking-1" },
            { config: { findActive: async () => rows } },
        );
    ...
    test("centralizes generic product tax, fee, discount and token calculations", async () => {
        const quote = await FinancialEngine.calculateQuote({
            subtotalMinor: 1_000_000,
            taxRateBasisPoints: 1800,
            discountRateBasisPoints: 1000,
            feeRatesBasisPoints: { service: 500 },
            fixedTokenMinor: 200_000,
            config: { token: { rateBasisPoints: 1500 } },
        });
    ...
    test("derives two tour package prices from reusable cost components", async () => {
                ...
                components: [
                    {
                        ...
                        pricing: {
                            ...
                            costAmountMinor: 200000,
                            sellingAmountMinor: 250000,
                        },
                    },
                    {
                ...
                packages: [
                    {
                        ...
                        tier: "BASIC",
                        name: "Basic",
                        includedComponentKeys: ["hotel-basic", "transfer"],
                        optionalComponentKeys: ["hotel-premium"],
                    },
                    {
                        packageKey: "premium",
                        tier: "PREMIUM",
                        name: "Premium",
                        includedComponentKeys: ["hotel-premium", "transfer"],
        ...
        const result = await FinancialEngine.calculateBookingFinancials({
            tour,
            packageKey: "basic",
            selections: { adults: 2, rooms: 1, nights: 2, vehicles: 1 },
    ...
    test("charges hotel upgrades as a difference and rejects unconfigured add-ons", async () => {
                ...
                components: [
                    {
                        ...
                        pricing: {
                            unit: "PER_ROOM",
                            costAmountMinor: 100000,
                            sellingAmountMinor: 150000,
                        },
                    },
                    {
                        componentKey: "upgrade",
                        type: "ACCOMMODATION",
                        name: "Upgrade",
                        replacesComponentKey: "base",
                        pricing: {
                            unit: "PER_ROOM",
                            costAmountMinor: 160000,
                            sellingAmountMinor: 240000,
                        },
                    },
                ],
                packages: [
                    {
                        ...
                        tier: "BASIC",
                        name: "Basic",
                        includedComponentKeys: ["base"],
                        optionalComponentKeys: ["upgrade"],
                    },
                    {
                        packageKey: "premium",
                        tier: "PREMIUM",
                        name: "Premium",
                        includedComponentKeys: ["upgrade"],
                        optionalComponentKeys: [],
                    },
                ],
            },
        };
        const priced = await FinancialEngine.calculateBookingFinancials({
            tour,
            packageKey: "basic",
            selections: { adults: 1, rooms: 1, optionalComponentKeys: ["upgrade"] },
            config: config({ commission: { enabled: false }, platformGst: { enabled: false } }),
        });
        ...
        await expect(
            FinancialEngine.calculateBookingFinancials({
                tour,
                packageKey: "basic",
                selections: { adults: 1, optionalComponentKeys: ["missing"] },
                config: config(),
            }),
    ...
    test("derives package totals from supplier costs plus agent fee and GST", async () => {
                ...
                packages: [
                    {
                        ...
                        tier: "BASIC",
                        name: "Base",
                        includedComponentKeys: ["hotel", "activity"],
                    },
                ],
            },
        };
        const result = await FinancialEngine.calculateBookingFinancials({
            tour,
            packageKey: "base",
            selections: { adults: 2, rooms: 1, nights: 3 },
            config: config({ commission: { enabled: false }, platformGst: { enabled: false } }),
        });
