import {
    getEligibleDeparturePrices,
    getLegacyTourPrice,
} from "../../modules/tours/search/tourSearch.eligibility.js";
...
const baseTour = {
    maxGroupSize: 30,
    group: { min: 1, max: 30 },
    price: { min: 89999, max: 89999, currency: "INR" },
};
...
describe("tour discovery search rules", () => {
    test("preserves explicit tag ids and derives domestic metadata from an India destination", () => {
        ...
        expect(metadata.searchTags).toContainEqual(
            expect.objectContaining({ slug: "domestic", type: "THEME" }),
        );
    });
    ...
    test("date-specific price excludes a tour whose matching departure exceeds max price", () => {
        const departures = [
            ...
            },
            {
                ...
                availableSeats: 20,
                pricing: { min: 94999, max: 94999 },
            },
        ];
        const prices = getEligibleDeparturePrices(baseTour, departures, {
            departureDate: "2026-12-20",
            returnDate: "2026-12-25",
            price: { max: 100000 },
        });
    ...
    test("remaining seats override the configured group maximum", () => {
        ...
        expect(
            getEligibleDeparturePrices(baseTour, departures, { travellers: 4, price: {} }),
        ).toEqual([]);
    });

    test("builds one database aggregation ending in a combined facet", () => {
        const normalized = normalizeTourSearchRequest({
            filters: { destinationCityIds: ["dubai"], travellers: 2 },
            sort: "recommended",
        }).value;
        ...
        expect(pipeline.at(-1).$facet).toEqual(
            expect.objectContaining({
    ...
    test("builds published text, origin, destination, country, tag, featured, duration, and price filters", () => {
        const search = normalizeTourSearchRequest({
            ...
            filters: {
                originCityIds: ["delhi"],
                destinationCityIds: ["dubai"],
                countryIds: ["uae"],
                agencyIds: ["agency-north"],
                tagIds: ["family"],
                featured: true,
                price: { min: 50000, max: 150000 },
                duration: { minDays: 4, maxDays: 8 },
                travellers: 2,
                departureDate: "2026-12-20",
                returnDate: "2026-12-25",
            },
        ...
        expect(pipeline[0].$match).toEqual(
            expect.objectContaining({ status: "published", $text: { $search: "desert" } }),
        );
        [
            "delhi",
            "dubai",
        ...
        ].forEach((value) => expect(serialized).toContain(value));
    ...
    test("filters departure and return dates and rejects unavailable departures", () => {
        ...
        expect(
            getEligibleDeparturePrices(baseTour, departures, {
                departureDate: "2026-12-20",
                returnDate: "2026-12-25",
                price: {},
            }),
    ...
    test("applies the seasonal price for legacy records", () => {
        ...
        expect(getLegacyTourPrice(tour, "2026-12-20")).toEqual(
            expect.objectContaining({ min: 129999, max: 149999 }),
        );
    });
    ...
    test("maps empty aggregation results into a stable paginated DTO", () => {
        ...
        expect(mapTourSearchResult({}, search)).toEqual(
            expect.objectContaining({
                items: [],
