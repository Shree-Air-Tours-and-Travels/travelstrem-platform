...
jest.unstable_mockModule("../../modules/auth/models/MobileOtpChallenge.js", () => ({ default: challengeModel }));
jest.unstable_mockModule("../../modules/auth/providers/mobile/index.js", () => ({ mobileOtpProvider: { sendOtp } }));
jest.unstable_mockModule("../../modules/auth/services/identity.service.js", () => ({ AuthServiceError, authenticateWithMobile }));

const { normalizeMobileNumber, requestMobileOtp, verifyMobileOtp } = await import("../../modules/auth/services/mobileAuth.service.js");
...
test("an unavailable OTP provider creates neither a challenge nor an authenticated user", async () => {
  sendOtp.mockRejectedValue(new AuthServiceError(
    "MOBILE_OTP_PROVIDER_NOT_CONFIGURED",
    "Mobile verification is currently unavailable.",
    503,
  ));

  await expect(requestMobileOtp({ phoneNumber: "+919876543210", portal: "customer" }))
...
test("verified OTP authenticates and cannot be reused", async () => {
  ...
  await expect(verifyMobileOtp({ challengeId: "507f1f77bcf86cd799439011", otp: "123456", portal: "customer" }))
...
test("expired and incorrect OTPs are rejected and attempts are counted", async () => {
  await requestMobileOtp({ phoneNumber: "+919876543210", portal: "customer" });
  await expect(verifyMobileOtp({ challengeId: "507f1f77bcf86cd799439011", otp: "999999", portal: "customer" }))
    .rejects.toMatchObject({ code: "INVALID_OTP" });
