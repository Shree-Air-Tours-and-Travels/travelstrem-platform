...
const { generateQuoteDocumentKey, sanitizePathSegment } =
    await import("../../services/r2/objectKey.js");
...
describe("objectKey helpers", () => {
    ...
    describe("generateQuoteDocumentKey", () => {
        test("generates correct key with all fields", () => {
            const key = generateQuoteDocumentKey({
                agencyId: "ag_123",
                bookingId: "bk_456",
                version: 2,
            });
...
describe("DocumentStorageService (configured)", () => {
    ...
    test("getSignedUploadUrl returns url and key", async () => {
        const result = await DocumentStorageService.getSignedUploadUrl(
            "quotes/test.pdf",
            "application/pdf",
        );
        expect(result.url).toContain("https://");
...
describe("DocumentStorageService (unconfigured)", () => {
    ...
    test("upload throws when not configured", async () => {
        await expect(
            DocumentStorageService.upload({ key: "test", body: Buffer.from("x") }),
        ).rejects.toThrow("R2 storage is not configured");
    });

    test("download throws when not configured", async () => {
        await expect(DocumentStorageService.download("test")).rejects.toThrow(
            "R2 storage is not configured",
        );
    });

    test("delete throws when not configured", async () => {
        await expect(DocumentStorageService.delete("test")).rejects.toThrow(
            "R2 storage is not configured",
        );
    });

    test("exists throws when not configured", async () => {
        await expect(DocumentStorageService.exists("test")).rejects.toThrow(
            "R2 storage is not configured",
        );
    });

    test("getSignedDownloadUrl throws when not configured", async () => {
        await expect(DocumentStorageService.getSignedDownloadUrl("test")).rejects.toThrow(
            "R2 storage is not configured",
        );
    });
    ...
    test("getSignedUploadUrl throws when not configured", async () => {
        await expect(
            DocumentStorageService.getSignedUploadUrl("test", "application/pdf"),
        ).rejects.toThrow("R2 storage is not configured");
    });
});
