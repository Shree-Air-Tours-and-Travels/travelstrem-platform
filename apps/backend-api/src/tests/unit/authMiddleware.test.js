...
jest.unstable_mockModule("../../modules/auth/models/PartnerAgency.js", () => ({ default: agencyModel }));
...
test("valid portal-scoped session attaches the internal user", async () => {
  const token = jwt.sign({ sub: "507f1f77bcf86cd799439011", portal: "customer", tokenVersion: 0 }, accessSecret, { expiresIn: "5m" });
  const names = getPortalCookieNames("customer");
  lean.mockResolvedValue({ role: "member", tokenVersion: 0, accountStatus: "active", agencyId: null });
  const req = { headers: { "x-travelstrem-portal": "customer" }, cookies: { [names.access]: token } };
  const next = jest.fn();
...
test("a session cannot be used against another portal", async () => {
  ...
  const res = response();
  await authMiddleware({ headers: { "x-travelstrem-portal": "admin" }, cookies: { [names.access]: token } }, res, jest.fn());
  expect(res.statusCode).toBe(401);
