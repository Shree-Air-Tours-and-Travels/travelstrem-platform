...
export default function FlightFiltersView({
  loading,
  error,
  searchError,
  pageModel,
  ...
  const searchFieldErrors = Object.fromEntries(
    Object.entries(searchError?.details || {}).map(([path, message]) => [
      fieldPathForError(path),
      message,
    ]),
  );
  const renderSearch = (placement, handleSearch = onSearch) => (
    <GlobalSearchCard
      ...
      choiceGroupsByMode={
        options[searchProps.choiceGroupsOptionsRef] || searchProps.choiceGroupsByMode
      }
  ...
  return (
    <main className="trehub-flight-list">
      <div className="trehub-flight-list__breadcrumb trehub-page__breadcrumbs">
        <Breadcrumbs
          items={(breadcrumbProps.items || []).map((item) => ({
      ...
      <OptionCardRail
        ...
        items={(getByPath(data, airlineProps.itemsDataPath) || []).map((item) => ({
          ...item,
          description: formatLabel(labelFor(labels, airlineProps.countTemplateRef), {
            count: item.count,
          }),
        }))}
        ...
        <section className="trehub-flight-list__results">
          <header>
            <div>
              ...
              <p>
                {formatLabel(labelFor(labels, resultProps.countTemplateRef), {
            ...
            <div className="trehub-flight-list__sort">
              <SingleSelect
                ...
                options={resolveOptions(resultProps.sortOptionsRef)}
          ...
          {results.length ? (
            results.map((flight) => (
              <FlightCard
                key={flight.offerId || flight.id}
                flight={flight}
                onSelect={onSelectFlight}
                labels={{
                  departureLabel: labelFor(labels, resultProps.departureLabelRef),
                  cabinLabel: labelFor(labels, resultProps.cabinLabelRef),
                  priceLabel: labelFor(labels, resultProps.priceLabelRef),
                  priceSuffix: labelFor(labels, resultProps.priceSuffixRef),
                  selectLabel: labelFor(labels, resultProps.selectLabelRef),
                }}
              />
            ))
          ) : (
            <NoDataFound
              title={labelFor(labels, resultProps.emptyTitleRef)}
              description={labelFor(labels, resultProps.emptyDescriptionRef)}
              icon="plane"
            />
          )}
          <Pagination
            ...
            previousLabel={labelFor(labels, resultProps.previousLabelRef)}
            nextLabel={labelFor(labels, resultProps.nextLabelRef)}
            ariaLabel={labelFor(labels, resultProps.paginationAriaRef)}
            disabled={loading}
            showSinglePage={Boolean(resultProps.showSinglePage && pagination.total > 0)}
          />
        </section>
      </div>
