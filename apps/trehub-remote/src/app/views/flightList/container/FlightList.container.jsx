...
import FlightListView from "../view/FlightList.view.jsx";
...
const FILTER_KEYS = ["airlines", "cabin", "stops", "maxPrice", "departureAfter", "arrivalBefore", "maxDuration", "refundable", "baggage"];
const SEARCH_KEYS = ["choice", "from", "to", "departDate", "returnDate", "segments", "travellers", "adults", "children", "infants", "currency", "directOnly"];

export default function FlightListContainer() {
  const [searchParams, setSearchParams] = useSearchParams();
  const location = useLocation();
  const navigate = useNavigate();
  const [state, setState] = useState({ loading: true, error: null, searchError: null, pageModel: null });
  ...
  useEffect(() => {
    ...
    const run = async () => {
      ...
      if (!searchIdRef.current || searchKeyRef.current !== nextSearchKey) {
        response = await createComponentData(pageModel.urls.flightSearch || "/flights/search", query);
        searchIdRef.current = response?.data?.searchId || "";
        searchKeyRef.current = nextSearchKey;
      } else {
        const path = (pageModel.urls.flightResults || "/flights/search/{searchId}/results").replace("{searchId}", encodeURIComponent(searchIdRef.current));
        response = await readComponentData(path, Object.fromEntries(Object.entries(query).filter(([key]) => !SEARCH_KEYS.includes(key))));
      }
      if (response?.status !== "success" || !response?.data) throw new Error("flight-results-invalid");
      if (!active) return;
    ...
    run()
      .catch((error) => {
        ...
        setState({ loading: false, error: error?.message || "flight-list-failed", searchError: null, pageModel: null });
      });
    return () => { active = false; };
  }, [queryKey]);
  ...
  return (
    <FlightListView
      ...
      onSelectFlight={(flight) => navigate(
