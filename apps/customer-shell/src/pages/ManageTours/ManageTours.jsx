...
import CreateTourForm from "../../components/forms/tour-crud-form/CreateTourForm";
import TourCardSecondary from "../Tour/Cards/TourCardSecondary";
...
export default function ManageTours() {
    ...
    async function fetchTours() {
        ...
        try {
            ...
            // normalize possible backend shapes into an array of tours
            // common shapes we've seen:
            // 1) sendSuccess -> res.componentData.data -> [ ...tours ]
            // 2) legacy -> res.componentData.state.data.tours -> [ ...tours ]
            // 3) direct -> res.data or res.tours (less common)
            let fetched = [];

            if (res && res.status === "success") {
                const cd = res.componentData || {};
                // common: componentData.data === array
                if (Array.isArray(cd.data)) {
                    fetched = cd.data;
                }
                // some responses nest at componentData.state.data.tours
                else if (cd.state && cd.state.data && Array.isArray(cd.state.data.tours)) {
                    fetched = cd.state.data.tours;
                }
                // fallback: componentData.data might hold an object with tours key
                else if (cd.data && Array.isArray(cd.data.tours)) {
                    fetched = cd.data.tours;
                }
                // last-ditch: res.data might be already the array
                else if (Array.isArray(res.data)) {
                    fetched = res.data;
                }
                // try a few other places gracefully
                else if (cd.data && Array.isArray(cd.data.items)) {
                    fetched = cd.data.items;
                } else {
                    // try to look for any array inside componentData
                    const maybeArray = Object.values(cd).find(v => Array.isArray(v));
                    if (maybeArray) fetched = maybeArray;
                }
            } else {
                throw new Error(res?.message || "Failed to fetch tours (non-success response)");
            }
    ...
    async function handleDelete(id) {
        ...
        try {
            const payload = { id };
            // use the correct endpoint path your backend expects
            const res = await fetchData(`/tours.json/${id}`, {
                method: "DELETE",
                headers: { "Content-Type": "application/json" },
                body: JSON.stringify(payload),
            });
            if (!res || res.status !== "success") throw new Error(res?.message || "Delete failed");
            await fetchTours();
        } catch (e) {
    ...
    async function handleDeleteAll() {
        ...
        try {
            ...
            if (!res || res.status !== "success") throw new Error(res?.message || "Delete all failed");
            await fetchTours();
        } catch (e) {
    ...
    async function fetchBookings() {
        ...
        try {
            // Admin/Agent sees all bookings; optionally pass filters later
            ...
            // backend may return array or object with bookings -> normalize
    ...
    async function handleConfirmBooking(bookingId, finalPriceData = {}) {
        try {
            // finalPriceData should be like { method, providerId, amountPaid, currency }
            const res = await fetchData(`/bookings/${bookingId}/confirm`, {
                method: "POST",
                headers: { "Content-Type": "application/json" },
    ...
    async function handleCancelBooking(bookingId) {
        try {
            const res = await fetchData(`/bookings/${bookingId}/cancel`, { method: "POST" });
