...
function AgencySection({ application, loading, auth, onApply, onRefresh }) {
  ...
  const handleSubmit = async (e) => {
    ...
    if (!form.agencyName.trim()) {
      setSubmitError("Agency name is required.");
      return;
    }
    setSubmitting(true);
    try {
      ...
      <div className="agency-section">
        ...
        <div className="agency-status-card">
          ...
          <dl className="agency-details">
            <div>
              <dt>Agency Name</dt>
              <dd>{app.agencyName || auth.user?.partnerAgencyRef || "-"}</dd>
            </div>
            <div>
              <dt>Agency Ref</dt>
              <dd>{app.partnerAgencyRef || auth.user?.partnerAgencyRef || "-"}</dd>
            </div>
            <div>
              <dt>Contact</dt>
      ...
      <form className="agency-form" onSubmit={handleSubmit}>
        ...
        <label className="agency-form__field">
          ...
          <input
            ...
            placeholder={auth.user?.email || "Email address"}
          />
        </label>
        <label className="agency-form__field">
      ...
      <div className="agent-profile-grid">
        <article className="agent-profile-card-large">
          <span className="agent-profile-card-large__avatar">
            {(data.name || auth.user?.name || "A").charAt(0).toUpperCase()}
          </span>
          <div>
            <Title
              text={profileLoading ? "Loading profile" : data.name || auth.user?.name || "Agent"}
            />
        ...
        <dl className="agent-profile-summary">
          <div>
            <dt>Role</dt>
            <dd>{data.role ?? auth.role}</dd>
          </div>
          <div>
            <dt>Status</dt>
            <dd>{data.approvalStatus ?? "approved"}</dd>
          </div>
          <div>
            <dt>Agency Ref</dt>
            <dd>{data.agencyRef ?? "independent"}</dd>
          </div>
          <div>
            <dt>Agent Ref</dt>
            <dd>{data.agentRef ?? "unassigned"}</dd>
          </div>
        </dl>
      </div>
      <AgencySection
