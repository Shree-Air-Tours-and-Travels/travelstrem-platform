...
export default function Header() {
  ...
  const handleLogout = useCallback(async () => {
    ...
    await authService.logout().catch((error) => {
      console.warn(
        "[Header] backend logout failed, clearing local session:",
        error?.message || error,
      );
    });
    ...
    if (configuredEvent !== "USER_LOGOUT")
  ...
  const handleNavigate = useCallback(
    async (item, path) => {
      const eventName =
        item?.event || EVENT_BY_PATH[path] || EVENT_BY_PATH[String(path || "").replace(/\/$/, "")];
  ...
  const augmentedConfig = {
    ...headerConfig,
    menu: [
      ...(Array.isArray(headerConfig.menu)
        ? headerConfig.menu.filter((item) => MENU_ALLOWLIST.has(item.id))
        : []),
      SERVICES_DROPDOWN,
    ],
  };
