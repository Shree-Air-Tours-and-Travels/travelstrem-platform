...
const revealItems = document.querySelectorAll('section, .product-card, .engine-grid article, .journal-card, .people-grid article, .stack > div');
...
heroArt?.addEventListener('pointermove', (event) => {
  ...
  heroArt.style.setProperty('--pointer-x', `${((event.clientX - bounds.left) / bounds.width - 0.5) * 2}`);
  heroArt.style.setProperty('--pointer-y', `${((event.clientY - bounds.top) / bounds.height - 0.5) * 2}`);
});
...
document.querySelectorAll('[data-coming-soon]').forEach((card) => {
  card.addEventListener('click', (event) => {
    ...
    if (statusCopy) statusCopy.textContent = `${product} is currently being developed for the Shree Air Tours and Travels ecosystem. We’ll share more when it is ready.`;
...
newsletterForm?.addEventListener('submit', (event) => {
  event.preventDefault();
  const email = new FormData(newsletterForm).get('email') || document.querySelector('#newsletter-email')?.value;
  if (!email) return;
  if (newsletterStatus) newsletterStatus.textContent = 'Thanks — your email app will open to complete the subscription.';
